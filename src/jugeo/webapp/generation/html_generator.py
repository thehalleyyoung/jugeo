    ...
    def _enrich_component_variety(self, spec: HTMLAppSpec, result: ObligationResult) -> HTMLAppSpec:
        ...
        if ComponentKind.HERO not in existing:
            needed.append(ComponentSpec(kind=ComponentKind.HERO, id="enriched-hero", props={
                "title": spec.title, "subtitle": f"An interactive {spec.title.lower()} experience",
...
def _generate_base_css(theme: dict[str, str]) -> str:
    ...
    *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    #loading-screen, .loading-screen {{
      ...
      background: var(--color-bg, #0a0a1a); color: var(--color-text, #e2e8f0);
      transition: opacity 0.5s ease;
    }}
    .loading-screen__content {{ text-align: center; }}
    .loading-screen__title {{ font-size: 2rem; font-weight: 700; margin: 1rem 0 0.5rem; }}
    .loading-screen__subtitle {{ color: var(--color-text-muted, #94a3b8); margin-bottom: 2rem; }}
    .loading-screen__progress-track {{
      width: 300px; height: 6px; background: var(--color-border, #2d3748);
      border-radius: 3px; overflow: hidden; margin: 0 auto 1rem;
    }}
    .loading-screen__progress-fill {{
      height: 100%; width: 0; background: var(--color-primary, #6366f1);
      border-radius: 3px; transition: width 0.3s ease;
    }}
    .loading-screen__progress-label {{ font-size: 0.85rem; color: var(--color-text-muted, #94a3b8); }}
    .loading-screen__steps {{ list-style: none; padding: 0; margin-top: 1.5rem; }}
    .loading-screen__step {{
      font-size: 0.85rem; padding: 0.25rem 0;
      color: var(--color-text-muted, #94a3b8); transition: color 0.3s;
    }}
    .loading-screen__step.done {{ color: var(--color-success, #10b981); }}
    .loading-screen__step.done::before {{ content: '✓ '; }}

    /* ─── View visibility for SPA routing ─── */
...
def _generate_base_js() -> str:
    """Generate base JavaScript for interactivity, loading, and SPA routing."""
    ...
    (function() {
      ...
      if (!screen) return;
      const fill = screen.querySelector('[data-hook="loading-progress-fill"]');
      const label = screen.querySelector('[data-hook="loading-progress-label"]');
      const steps = screen.querySelectorAll('[data-hook="loading-steps"] li, .loading-screen__step');
      const bar = screen.querySelector('[data-hook="loading-progress"]');
      let pct = 0;
      const phases = ['engine', 'canvas', 'audio', 'ai', 'data'];
      let idx = 0;
      function advance() {
        if (idx < phases.length) {
          ...
          idx++;
          pct = Math.round((idx / phases.length) * 100);
    ...
    if ('IntersectionObserver' in window) {
      ...
      document.querySelectorAll('.fade-in,.animate-fade-up').forEach(el => obs.observe(el));
    }
    ...
    (function() {
      function showView(viewName) {
        // Hide all data-view sections
        document.querySelectorAll('[data-view]').forEach(el => {
          el.style.display = 'none';
        });
        // Show the target view (or home/play by default)
        const target = document.querySelector('[data-view="' + viewName + '"]');
        if (target) {
          target.style.display = '';
        } else {
          // Show first view or home
          const home = document.querySelector('[data-view="home"]') ||
                       document.querySelector('[data-view="play"]') ||
      ...
      router.on('/', () => showView('home'));
      router.on('/play', () => showView('play'));
      router.on('/gallery', () => showView('gallery'));
      router.on('/tutorial', () => showView('tutorial'));
      router.on('/scores', () => showView('scores'));
      router.on('/settings', () => showView('settings'));
      router.on('/about', () => showView('about'));
      router.on('*', (path) => showView(path.replace('/', '') || 'home'));

      // Start router after DOM is ready
    ...
    document.addEventListener('click', e => {
      ...
      if (action === 'navigate') {
        ...
        if (target) window._jugeoRouter.navigate(target.replace('#', ''));
        ...
        window._jugeoRouter.navigate('/play');
        if (window.CT && window.CT.GameEngine) {
          try { new window.CT.GameEngine().init(); } catch(e) { console.warn('Game init:', e); }
        }
      }
    });
    ...
    console.log('jugeo-webapp initialized');
    """)
...
class HTMLOnlyGenerator:
    ...
    def _generate_page(self, spec: HTMLAppSpec, page: PageSpec) -> str:
        ...
        <head>
          ...
          <meta name="description" content="{spec.title}" />
