...
def _derive_tagline(concepts: ConceptMap) -> str:
    """Build a user-facing tagline from the concept map (never echo the raw prompt)."""
    domains = [d.value.title() for d in concepts.domains]
    if len(domains) > 2:
        return f"{', '.join(domains[:2])} & more — interactive experience"
    if domains:
        return f"An interactive {' & '.join(domains).lower()} experience"
    return f"An interactive {concepts.app_title.lower()} experience"
...
class PromptToApp:
    ...
    def _build_components(self, all_html: str) -> list[ComponentSpec]:
        """Build page components from agent HTML or template fallback.

        When *all_html* contains agent-generated body content it is used
        as the primary page structure.  The template only adds a footer
        — it does NOT wrap the agent HTML in hero/card/tabs that would
        conflict with the agent’s own structure.
        """
        comps: list[ComponentSpec] = []

        if all_html:
            # Agent HTML IS the page structure — embed directly
            comps.append(ComponentSpec(
                kind=ComponentKind.CUSTOM, id="app-content",
                custom_html=all_html,
            ))
        else:
            # Template fallback: build a basic page structure
            tagline = _derive_tagline(self.concepts)
            comps.append(ComponentSpec(kind=ComponentKind.HERO, id="hero", props={
                "title": self.concepts.app_title,
                "subtitle": tagline,
            }))
            ...
            comps.append(ComponentSpec(kind=ComponentKind.CARD, id="info-card"))
            comps.append(ComponentSpec(kind=ComponentKind.TABS, id="mode-tabs", props={
        ...
        comps.append(ComponentSpec(kind=ComponentKind.FOOTER, id="footer", props={
            "text": f"\u00a9 {self.concepts.app_title}",
        }))
        return comps
    ...
    def _build_content_components(self) -> list[ComponentSpec]:
        ...
        faq_items = [
            {"title": "What is this?",
             "content": f"{self.concepts.app_title} — an interactive application "
                        "combining multiple feature domains into a unified experience."},
            {"title": "How does it work?",
             "content": "The application uses client-side rendering with HTML5 Canvas, "
                        "CSS animations, and modular JavaScript for a smooth, "
