...
% Depends on: \input{jugeo-common} and \input{experiment-data}
...
The following listing shows how to reproduce the benchmark run for a
single program using the \JuGeo{} Python API:

\begin{lstlisting}[language=Python, caption={Running the full \JuGeo{} verification pipeline on a benchmark program.}]
from jugeo.geometry import SiteBuilder

code = open("benchmark/sorting/merge_sort.py").read()
...
# Trigger the full verification pipeline and inspect results
suite = site.benchmark_suite()
print(f"Verdict:     {suite['verdict']}")
print(f"Trust tier:  {suite['trust']}")
print(f"Props total: {suite['propositions_total']}")
print(f"Props OK:    {suite['propositions_discharged']}")
\end{lstlisting}
...
\begin{lstlisting}[language=Python, caption={Orchestrating the full \JuGeo{} verification pipeline and inspecting the evaluation design.}]
from jugeo.geometry import SiteBuilder

code = open("benchmark/state_machines/traffic_light.py").read()
site = SiteBuilder(code).build()

print(f"Coordinates: {site.coordinate_count()}")
print(f"Covering families: {len(site.covering_families())}")

# Run the full orchestration pass
report = site.orchestrate_verification()
print(f"Verdict:      {report['verdict']}")
print(f"Trust level:  {report['trust_level']}")
print(f"Obstructions: {report['obstruction_count']}")

# Inspect the evaluation design for the benchmark
design = site.evaluation_design()
for domain, metrics in design['domain_breakdown'].items():
    print(f"  {domain}: coords={metrics['mean_coords']:.1f},"
          f" props={metrics['mean_props']:.1f},"
          f" time={metrics['mean_time_ms']:.1f}ms")
\end{lstlisting}
...
\begin{lstlisting}[language=Python, caption={Comparing \JuGeo{} semantic-site diagnostics against a baseline analysis for a failing program.}]
import subprocess, json

# Run JuGeo in JSON mode to retrieve structured comparison data
result = subprocess.run(
    ["python3", "-m", "jugeo", "--format", "json",
     "verify", "failing_program.py"],
    capture_output=True, text=True
)
data = json.loads(result.stdout)

# Inspect the obstruction coordinates (H^1 support)
obstructions = data.get("obstructions", [])
print(f"H^1 non-vanishing at {len(obstructions)} coordinate(s):")
for obs in obstructions:
    print(f"  coord={obs['coordinate']}  overlap={obs['overlap']}")
    print(f"  cocycle={obs['cocycle_class']}  trust={obs['trust']}")

# Show trust-level distribution across judgments
for trust, count in data.get("trust_distribution", {}).items():
    print(f"  {trust}: {count} judgment(s)")
\end{lstlisting}
