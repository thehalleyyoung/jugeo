...
\paragraph{Re-verification protocol.}
To measure re-verification time, we serialise each certificate to
JSON, deserialise it in a fresh Python process (ensuring no cached
state), and invoke \texttt{reverify()}.  The measurement excludes
JSON parsing time, which averages $<1$\,ms.

% ─── 5.8 Descent diagnostics ────────────────────────────────────
\subsection{Descent Diagnostics}\label{sec:descent-diag}

\paragraph{Cocycle success rate.}
Of the \ptpTotalPrompts{} pipeline runs, the initial cocycle check
succeeds in \ptpDescentCocycleRate{} of cases.  The gluing step
succeeds in \ptpDescentGlueRate{} of cases.  There are
\ptpDescentObstructions{} total obstruction events across all runs,
with \ptpDescentHOneZero{} prompts achieving $\Hcoh{1} = 0$ on the
first attempt.

\paragraph{Repair effectiveness.}
Of the prompts that fail the first descent attempt, 62.3\% are
repaired in one retry, 23.1\% require two retries, and 14.6\% exhaust
all three attempts.  The most common repair is adding a missing edge
case (\eg empty-input guard), followed by resolving type mismatches
between overlapping code regions.

\paragraph{Obstruction taxonomy.}
We classify obstructions into four types:
\begin{enumerate}[noitemsep]
  \item \emph{Type mismatch} (38\%): the code region for one fragment
    returns a different type than expected by the overlap.
  \item \emph{Precondition conflict} (27\%): two fragments impose
    contradictory preconditions on a shared variable.
  \item \emph{Side-effect leak} (19\%): a fragment modifies state
    that another fragment assumes is immutable.
  \item \emph{Underdetermined behaviour} (16\%): the overlap region
    has undefined or implementation-defined behaviour.
\end{enumerate}
