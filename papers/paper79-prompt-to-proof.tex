...
\usepackage{lmodern}
\input{jugeo-common}
% data file removed (no real experiment)
...
All core theorems are formalised in \LEAN{}.
...
This section describes the evaluation methodology for \PromptToProof{}.
Empirical results on a large-scale benchmark are left to future work.

\paragraph{Proposed benchmark design.}
An evaluation of \PromptToProof{} would measure certificate generation
rate (fraction of prompts for which the five-stage pipeline produces a
full sheaf certificate), false-positive rate (certificates issued for
incorrect code), and re-verification speedup.  Appropriate benchmark
tasks include natural language programming prompts paired with
reference implementations, spanning arithmetic, string processing,
list manipulation, graph algorithms, and data structure operations.

\paragraph{Pipeline stages under evaluation.}
The five pipeline stages (\IntentParse, \CodeGen, \SiteConstruct,
\DescentVerify, \CertEmit) each introduce latency and potential
failure points.  \IntentParse{} may misidentify requirement
boundaries; \DescentVerify{} is subject to SMT solver timeouts on
complex overlap constraints.  Ablation studies that skip individual
stages would quantify each component's contribution.

\paragraph{Certificate quality metrics.}
Beyond pass/fail, certificate quality includes: trust tier distribution
(how many judgments reach $\Tproof$ vs.\ $\Tsolver$ vs.\ $\Truntime$),
certificate size relative to source, and re-verification time.

\paragraph{Running example.}
We illustrate the pipeline on a single example:
...
\paragraph{Stage 5.}
\CertEmit{} produces a certificate of size 1.8\,KB with all
...
Our experiments use GPT-4 for code generation.  We conducted a
preliminary study with GPT-3.5 and observed a certificate rate of
71.3\%, compared to \ptpSuccessRate{} with GPT-4.  The decline is
attributable to lower code quality (more syntax errors, more logic
bugs) rather than to pipeline limitations.  As LLM capability
improves, the certificate rate should increase without pipeline
modifications.
...
All code, benchmarks, and experimental scripts are available in the
\JuGeo{} repository.  Certificates for the \ptpSuccessCert{}
successfully certified programs are included as JSON artifacts.
