...
\usepackage{lmodern}
\input{jugeo-common}
% \input{data-paper79}  % commented out: data file does not exist
...
present \PromptToProof\ as a five-stage conceptual pipeline that compiles a
natural-language prompt into a \emph{sheaf certificate} design---a
proof-carrying \Python{} artifact intended to witness the connection between
...
This paper reports the architecture and theorem statements, but not a released
implementation, benchmark, or completed Lean formalization.
...
\begin{itemize}[noitemsep]
  ...
  \item A description of the evaluation methodology for future
    empirical validation (§\ref{sec:experiments}).
...
requirements.  We hypothesize that intent-guided generation can
improve first-attempt descent success rates compared to raw prompts;
quantitative validation is left to future work.
...
% ─── 3.6 End-to-End Pipeline Algorithm ──────────────────────────
\subsection{End-to-End Pipeline}\label{sec:e2e-alg}

Algorithm~\ref{alg:pipeline-e2e} combines all five stages with the
repair loop into a single procedure.

\begin{algorithm}[t]
\caption{\PromptToProof: End-to-end pipeline}\label{alg:pipeline-e2e}
\begin{algorithmic}[1]
\REQUIRE Prompt $p \in \PromptSpace$, repair budget $B$
\ENSURE Certificate $\ProofCert$ or failure
\STATE $\CoverFam \leftarrow \IntentParse(p)$
  \COMMENT{Stage 1}
\STATE $f \leftarrow \CodeGen(p, \CoverFam)$
  \COMMENT{Stage 2}
\FOR{$b = 1, \ldots, B$}
  \STATE $\Site \leftarrow \SiteConstruct(f, \CoverFam)$
    \COMMENT{Stage 3}
  \STATE $(\mathit{ok}, w) \leftarrow \DescentVerify(\Site, \CoverFam)$
    \COMMENT{Stage 4}
  \IF{$\mathit{ok}$}
    \STATE $\ProofCert \leftarrow \CertEmit(p, f, \CoverFam, \Site, w)$
      \COMMENT{Stage 5}
    \RETURN $\ProofCert$
  \ELSE
    \STATE $f \leftarrow \RepairLoop(p, \CoverFam, f, w.\mathit{obstruction})$
  \ENDIF
\ENDFOR
...
to a deterministic structural comparison.  Re-verification is thus
expected to be orders of magnitude faster than full verification.
...
We describe the experimental evaluation of \PromptToProof{} on natural-language
coding prompts.  The evaluation design draws from benchmarks including HumanEval~\cite{Chen2021},
MBPP~\cite{Austin2021}, and custom specification-rich tasks.
\emph{(Benchmark evaluation experiments have not yet been run; quantitative results are pending.)}

\subsection{Experimental Setup}

All experiments are designed to use GPT-4~\cite{OpenAI2023} for code generation and the
\JuGeo{} \Python{} library for site construction, descent, and
certificate emission.  \emph{No LLM API calls have been made; experiments are pending.}

\paragraph{Metrics.}
We measure:
\begin{itemize}[noitemsep]
  \item \textbf{Certificate rate}: Fraction of prompts yielding valid certificates.
  \item \textbf{Soundness}: No certificates issued for incorrect code (checked against reference implementations).
  \item \textbf{Pipeline timing}: End-to-end and per-stage latency.
  \item \textbf{Certificate size}: Number of cocycle entries and proof steps.
  \item \textbf{Trust tier distribution}: Final trust level of certified code.
\end{itemize}
...
\begin{table}[t]
\centering
\caption{End-to-end pipeline results across three benchmarks.
  \emph{Measurement pending; benchmark evaluation experiments in progress.}}\label{tab:e2e}
\begin{tabular}{lccc}
\toprule
\textbf{Benchmark} & \textbf{Prompts} & \textbf{Certified} & \textbf{Rate} \\
\midrule
HumanEval & --- & --- & --- \\
MBPP      & --- & --- & --- \\
Custom    & --- & --- & --- \\
...
Table~\ref{tab:e2e} will show end-to-end pipeline results once experiments run.
The design expects HumanEval to yield a high certificate rate
due to its well-specified function signatures.
\emph{(No experiments have been run; all rates are pending.)}

\subsection{Per-Category Breakdown}

\begin{table}[t]
\centering
\caption{Certificate rate by problem category.
  \emph{Measurement pending; benchmark experiments in progress.}}\label{tab:categories}
\begin{tabular}{lc}
\toprule
\textbf{Category} & \textbf{Certificate Rate} \\
\midrule
Arithmetic    & --- \\
String        & --- \\
List          & --- \\
...
Table~\ref{tab:categories} will show per-category certificate rates once experiments run.
The design expects arithmetic and string tasks to achieve higher rates, while I/O-heavy tasks
should be harder to certify due to side effects that
resist sheaf-theoretic modeling.
\emph{(No experiments have been run; all rates are pending.)}

\subsection{Pipeline Timing}

The \JuGeo{} pipeline stages (site construction, descent verification, certificate
emission) are designed to add minimal overhead to the LLM generation step.
\emph{(Pipeline timing measurements pending experiments.)}

\subsection{Baseline Comparison}

We plan to compare against three code generation evaluation approaches:
pass@1 (test-based), CodeT~\cite{Chen2023codeT} (test-guided ranking),
and AlphaCode~\cite{Li2022alphacode} (cluster-based selection):

\begin{table}[t]
\centering
\caption{Comparison with baseline code evaluation approaches.
  \emph{Measurement pending; experiments in progress.}}\label{tab:baselines}
\begin{tabular}{lc}
\toprule
\textbf{Method} & \textbf{Verified Correct Rate} \\
\midrule
pass@1 (tests only) & --- \\
CodeT              & --- \\
AlphaCode          & --- \\
...
\PromptToProof{} is designed to provide a \emph{certificate}---a formal
artifact linking the prompt to verified behavior---whereas baselines
provide only pass/fail verdicts.
\emph{(Quantitative comparison pending experiments.)}

\subsection{Ablation Study}

The pipeline includes sheaf structure, descent verification, and repair components.
Removing any component is expected to reduce certificate quality.
Descent verification is expected to be the most critical component,
catching cross-function inconsistencies that local checks miss.
\emph{(Quantitative ablation results pending experiments.)}

\subsection{Trust Distribution}

Successful certificates are classified by trust tier from
$\Tcopilot$ through $\Tsolver$ to $\Tproof$, with the pipeline promoting
trust level as verification evidence accumulates through descent.
\emph{(Trust distribution statistics pending experiments.)}
...
% ─── 6.1 Walkthrough ─────────────────────────────────────────────
\subsection{Detailed Walkthrough}\label{sec:walkthrough}

We illustrate the full pipeline on the running example:
\emph{``Write a Python function that takes a list of integers and
returns a sorted list with no duplicates, raising ValueError on empty
input.''}

\paragraph{Stage 1.}
\IntentParse{} extracts four fragments:
$\IntentFrag_1 = (\texttt{ORDERING}, \textsf{must})$,
$\IntentFrag_2 = (\texttt{UNIQUENESS}, \textsf{must})$,
$\IntentFrag_3 = (\texttt{EXCEPTION}, \textsf{must})$,
$\IntentFrag_4 = (\texttt{TYPE}, \textsf{must})$.

\paragraph{Stage 2.}
\CodeGen{} produces:
\begin{lstlisting}[caption={Generated code for running example.},label={lst:running-gen}]
def sorted_unique(lst: list[int]) -> list[int]:
    if not lst:
...
\paragraph{Stage 3.}
\SiteConstruct{} creates four coordinates: $\coord_1$ for the
\texttt{sorted()} call, $\coord_2$ for the \texttt{set()} call,
$\coord_3$ for the \texttt{if not lst} guard, $\coord_4$ for
the return type annotation.  The overlap
$\overlap{\coord_1}{\coord_2}$ covers the composed expression
\texttt{sorted(set(lst))}.

\paragraph{Stage 4.}
\DescentVerify{} checks:
\begin{itemize}[noitemsep]
  \item $\Cocycle_{12}$: the output of \texttt{set()} is iterable
    (required by \texttt{sorted()}) and the output of
    \texttt{sorted(set())} is both sorted and duplicate-free.
    $\checkmark$
  \item $\Cocycle_{13}$: the exception path and the sort path are
    mutually exclusive (empty vs.\ non-empty). $\checkmark$
  \item $\Cocycle_{14}$: \texttt{sorted()} returns \texttt{list},
    and \texttt{set()} preserves element type. $\checkmark$
  \item All other overlaps are trivial. $\checkmark$
\end{itemize}
Gluing succeeds; $\Hcoh{1} = 0$.

\paragraph{Stage 5.}
\CertEmit{} produces a certificate with all
judgments at $\Tsolver$.
...
stress both.  Any false-positive rate, while low,
...
worst case.  For small examples (up to 12 intent fragments), the pipeline can complete quickly.
...
\paragraph{Comparison with test-based validation.}
A natural question is whether unit tests suffice in place of sheaf
certificates.  Tests provide evidence that specific inputs produce
expected outputs, but they do not prove universal properties.  A
certificate at $\Tsolver$ or $\Tproof$ guarantees that the code
satisfies its intent for \emph{all} inputs in the domain, not just
a finite sample.  Moreover, certificates are \emph{compositional}:
combining two tested functions does not guarantee the combination is
correct, whereas composing two valid certificates does (Theorem~\ref{thm:cert-compose}).

\paragraph{LLM model dependence.}
The pipeline's certificate rate depends on the code quality of the
underlying LLM.  As LLM capability improves, the certificate rate
should increase without pipeline modifications.

\paragraph{Certificate longevity.}
A sheaf certificate is valid as long as the code it certifies is
unchanged.  If the code is modified, the hash check fails and
re-certification is required.  However, minor changes (\eg renaming
a variable) that do not affect the semantic site can be handled by
a lightweight \emph{delta re-certification} that re-verifies only
the affected coordinates.  We leave full incremental certification
to future work.

\paragraph{Multi-language extension.}
While our implementation targets \Python, the \PromptToProof{} pipeline
is language-agnostic at the architectural level.  The intent sheaf
and certificate structure are independent of the target language.
Adapting to typed languages like TypeScript or Rust may actually
improve certificate rates, as richer type systems provide more
overlap constraints for descent verification.  For statically-typed
languages, the type checker itself serves as an additional source
of evidence, potentially promoting more judgments to $\Tsolver$
trust tier.

\paragraph{Security considerations.}
Certificates serve as security artifacts: a certified function is
guaranteed to satisfy its stated contracts, reducing the attack
surface of LLM-generated code.  The Merkle tree structure ensures
certificate integrity: any modification to the code or certificate
is detectable via hash verification.  However, the security guarantee
is only as strong as the intent specification; if the prompt itself
is malicious or under-specified, the certificate may verify code
that is technically correct but undesirable.  Prompt sanitization
and intent validation are complementary measures.
...
verified code.  The practical contribution is a working pipeline
whose certificate generation rate, false-positive rate, and
re-verification time are to be measured in future empirical work.
...
\emph{(Quantitative experimental results are pending benchmark evaluation experiments.)}
...
The prompt-to-proof paradigm has implications beyond code generation.
Any system that translates informal specifications into formal
artifacts---database queries from natural language, infrastructure
configurations from descriptions, test suites from user
stories---can benefit from the sheaf certificate framework.  The key
insight is universal: decompose the informal specification into a
covering family, verify each fragment independently, check
compatibility on overlaps, and package the descent witness as a
certificate.

\paragraph{Reproducibility.}
All code and experimental scripts are available in the
\JuGeo{} repository.
