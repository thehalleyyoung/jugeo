...
\newcommand{\merge}{\mathrm{merge}}            % merge operation
...
reporting $\le\!1\,\mathrm{ms}$ mean verification latency with zero
...
X.509 certificate chains \citeX509{} inspire our authority model, but
...
\midrule
Specification    & 100 & 0.62 & 1.10 & 1.43 \\
Equivalence      & 100 & 0.71 & 1.22 & 1.61 \\
Bug detection    & 100 & 0.58 & 0.98 & 1.19 \\
\midrule
\textbf{All}     & \textbf{300} & \textbf{0.64} & \textbf{1.11} & \textbf{1.61} \\
\bottomrule
\end{tabular}
\end{center}

All verifications complete in under $2\,\mathrm{ms}$, well within the
...
takes $0.21\,\mathrm{ms}$ on average, dominated by list allocation.
...
LLM-readable diagnostics in $0.38\,\mathrm{ms}$ per instance.
...
benchmark suite with $<\!2\,\mathrm{ms}$ per-chain verification latency,
