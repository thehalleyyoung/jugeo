...
\newcommand{\WeakR}{\mathsf{weak}}            % weakening rule
...
\newcommand{\CutR}{\mathsf{cut}}              % cut rule
...
JuGeo evaluation suite.  The engine resolves $87\,\%$ of all obligations
using at most three chained rules; average chain length is $1.8$ rules;
and the per-obligation latency is $0.9\,\mathrm{ms}$ at the
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{Annotate.}  The global trust tier is computed as
    $\tau = \bigtjoin_i \tau_i$ where $\tjoin$ is the conservative
...
\begin{proposition}[Trust Conservation under Gluing]\label{prop:trust-glue}
  ...
  tier $\tau_{\mathit{global}} = \bigtjoin_i \tau_i$ where $\tjoin$ is
...
\begin{tabular}{lrrrr}
  ...
  \midrule
  Specification     & \numspec  & 89\,\% & 1.7 rules & 0.8\,ms \\
  Equivalence       & \numeq    & 86\,\% & 2.0 rules & 1.1\,ms \\
  Bug detection     & \numbug   & 85\,\% & 1.8 rules & 0.9\,ms \\
  \midrule
  Total / average   & \numcases & 87\,\% & 1.8 rules & 0.9\,ms \\
  \bottomrule
\end{tabular}
...
$3.2$ rules and $13\,\%$ of cases required manual rule suggestion.
With schemas, the average chain length drops to $1.8$ and manual
intervention falls to $3\,\%$.

\paragraph{Unresolved cases.}
The $13\,\%$ of unresolved cases (across all suites) are escalated to
the Descent Engine, which uses the global gluing procedure to complete
the proof.  Of these, $78\,\%$ are resolved by the Descent Engine
within $\totaltime$ total time, yielding an end-to-end resolution
rate of $97\,\%$.

\paragraph{Trust tier distribution.}
Of resolved proofs, $71\,\%$ achieve $\Tsolver$ tier, $22\,\%$
achieve $\Tcopilot$ tier (where at least one leaf was a Copilot-oracle
inference), and $7\,\%$ remain at $\Tunver$ (insufficient local
...
$44\,\%$ and nearly eliminates the need for manual rule suggestion.
