...
JuGeo evaluation suite.  The current measurements resolve 10 benchmark
obligations in total, with average per-suite times between about 6 and
7 seconds and an overall average proof-chain length of 2.2.  This
audited version therefore keeps the rule-engine and soundness claims but
does not claim sub-millisecond performance.
\end{abstract}
...
\begin{tabular}{lrrrr}
  ...
  \midrule
  Specification     & \numspec  & \ppXXXIVSpecResolved & \ppXXXIVSpecChain & \ppXXXIVSpecTime \\
  Equivalence       & \numeq    & \ppXXXIVEquivResolved & \ppXXXIVEquivChain & \ppXXXIVEquivTime \\
  Bug detection     & \numbug   & \ppXXXIVBugResolved & \ppXXXIVBugChain & \ppXXXIVBugTime \\
  \midrule
  Total / average   & \numcases & \ppXXXIVTotalResolved & \ppXXXIVOverallChain & \subSiteMeanTime \\
  \bottomrule
\end{tabular}
...
\begin{tabular}{lr}
  ...
  \midrule
  $\MP$        & \ppXXXIVRuleMP \\
  $\ConjI$     & \ppXXXIVRuleConjI \\
  $\WeakR$     & \ppXXXIVRuleWeakR \\
  $\ConjE$     & \ppXXXIVRuleConjE \\
  $\SubstR$    & \ppXXXIVRuleSubstR \\
  $\UnivI$     & \ppXXXIVRuleUnivI \\
  $\FrameR$    & \ppXXXIVRuleFrameR \\
  \bottomrule
\end{tabular}
...
The measured benchmark records short proof chains (2.2 applications on
average for resolved cases), but the current repository data does not
include a separate ablation isolating the theorem-schema contribution.

\paragraph{Unresolved cases.}
The current table reports 10 locally resolved obligations.  This paper
does not make an additional audited claim about downstream Descent
Engine recovery for unresolved obligations.
