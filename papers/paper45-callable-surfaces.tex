...
\midrule
HOF          & \numhof   & \hofrecall  & 91.2 & 5.8  \\
Callback (CB) & \numcb    & \cbrecall   & 88.4 & 7.1  \\
Decorator    & 159        & 96.8        & 93.7 & 4.4  \\
\midrule
\textbf{Overall} & \numcases & \textbf{94.1} & \textbf{91.0} & \textbf{\medtime} \\
\bottomrule
\end{tabular}
\end{table}

Callable surface recall exceeds 94\% across all categories.
Decorators achieve the highest recall (96.8\%) because their syntactic
form is highly regular; callbacks achieve the lowest (91.8\%) due to
dynamic registration patterns in event-loop code that are not yet
handled by the current callee classifier.

Contract coverage is $91\%$ overall.  The $9\%$ gap consists
...
falls back to a conservative unverified contract.  Soundness rate is
$88.3\%$: programs with full contract coverage all satisfy the
theorem; the remaining $11.7\%$ have at least one surface with a
...
Disabling closure analysis reduces contract coverage by $7.3$
percentage points on programs with closures.  Disabling decorator
verification degrades soundness rate by $5.1$ points.  Disabling
callback chain composition reduces recall on asyncio programs by
$8.9$ points.  These ablations confirm that all three sub-analyses
...
Experiments on $\numcases{}$ programs show $94.1\%$ surface recall
and $91\%$ contract coverage at a median latency of $\medtime{}$
per program.

\paragraph{Limitations.}
The current callee classifier does not handle all forms of dynamic
callable construction; $9\%$ of surfaces receive conservative
