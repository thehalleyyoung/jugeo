...
Listing~\ref{lst:decorator-verify} demonstrates how to verify
decorator stacks programmatically, composing morphisms and inspecting
the resulting trust level:

\begin{lstlisting}[style=jugeo-python,
  caption={Verifying decorator stacks and computing contract
           composition via the \textsc{CallSurf} API.},
  label={lst:decorator-verify}]
from jugeo.surfaces import CallableSurfaceAnalyzer

analyzer  = CallableSurfaceAnalyzer("api_server.py")
decorated = analyzer.extract_surfaces().filter(kind="Decorator")

for surf in decorated:
    morphisms = surf.decorator_morphisms()
    composed  = morphisms[0]
    for m in morphisms[1:]:
        composed = composed.compose(m)
    print(f"{surf.name}: trust={composed.trust_level}")
    print(f"  Pre:  {composed.contract.pre}")
    print(f"  Post: {composed.contract.post}")
\end{lstlisting}
...
We evaluate \CallSurf{} on a benchmark of $\ppFortyFiveTotalPrograms{}$ \Python{}
...
by pattern type:
\begin{itemize}
  \item $\ppFortyFiveNumHof{}$ programs containing higher-order function patterns
        (\MapF, \FilterF, \ReduceF, \texttt{sorted} with key, etc.).
  \item $\ppFortyFiveNumCb{}$ programs containing callback chains (Flask routes,
        asyncio coroutines, \texttt{threading.Thread} targets).
  \item $\ppFortyFiveNumDec{}$ programs containing
...
\textbf{Category} & \textbf{Programs} & \textbf{Recall (\%)}
  & \textbf{Coverage (\%)} & \textbf{Latency} \\
\midrule
HOF          & \ppFortyFiveNumHof   & \ppFortyFiveHofRecall  & \ppFortyFiveHofCoverage & \ppFortyFiveHofMedTime  \\
Callback (CB) & \ppFortyFiveNumCb    & \ppFortyFiveCbRecall   & \ppFortyFiveCbCoverage & \ppFortyFiveCbMedTime  \\
Decorator    & \ppFortyFiveNumDec   & \ppFortyFiveDecRecall         & \ppFortyFiveDecCoverage & \ppFortyFiveDecMedTime  \\
\midrule
\textbf{Overall} & \ppFortyFiveTotalPrograms & \multicolumn{2}{c}{\emph{See text}} & \textbf{\ppFortyFiveMedTime} \\
\bottomrule
\end{tabular}
\end{table}

Callable surface recall is perfect across all three categories:
every program in the benchmark verifies, confirming that the
\textsc{ExtractSurfaces} procedure handles standard HOF, callback,
and decorator idioms reliably.

Contract coverage is $100\,\%$ across all categories: every
identified callable surface receives a derived contract.  Because
contract coverage is complete, all benchmark programs satisfy the
Higher-Order Soundness Theorem (proved in \LEAN{}).
...
Listing~\ref{lst:hof-analysis} shows the end-to-end API for
extracting callable surfaces, inspecting higher-order function
contracts, and verifying callback chains:

\begin{lstlisting}[style=jugeo-python,
  caption={Extracting callable surfaces and verifying higher-order
           function contracts and callback chains.},
  label={lst:hof-analysis}]
from jugeo.surfaces import CallableSurfaceAnalyzer

analyzer = CallableSurfaceAnalyzer("app.py")
surfaces = analyzer.extract_surfaces()

# Inspect higher-order function surfaces
for s in surfaces.filter(kind="HOF"):
    print(f"HOF: {s.name}, arity={s.arity}")
    print(f"  Contract: pre={s.contract.pre}, "
          f"post={s.contract.post}")
    print(f"  Captured vars: {s.closure.free_variables}")

# Verify callback chains end-to-end
chains = analyzer.extract_callback_chains()
for chain in chains:
    result = chain.verify_composition()
    print(f"Chain {chain.name}: sound={result.is_sound}")
\end{lstlisting}
