% paper73-multi-agent-consensus.tex --- Multi-Agent Consensus Through Descent: When Agents Disagree
%   Paper 73 of the Judgment Geometry series.
% Compile: pdflatex paper73-multi-agent-consensus
\documentclass[11pt]{article}
\usepackage{lmodern}
\input{jugeo-common}
% \input{data-paper73}  % data file does not exist: no measured data for paper 73
...
consensus.  We outline an evaluation methodology for measuring consensus rates,
conflict resolution, and code quality improvement across multiple agents,
with quantitative validation planned for future work.
\end{abstract}
...
\begin{itemize}
  ...
  \item An evaluation methodology for multi-agent consensus
...
To validate the framework, we propose evaluating on programming tasks
drawn from multiple domains.  Each task would be assigned to several
heterogeneous LLM-based agents working independently on overlapping
code coordinates.

\paragraph{Baselines.}
The planned evaluation compares four merge strategies:
(1)~best single agent; (2)~naive merge (concatenation);
(3)~majority voting; (4)~descent-based consensus.  The primary
metrics are bug count, consensus rate, and code quality improvement.
...
Number of bugs in the merged output under each strategy, comparing
single-agent, naive merge, majority voting, and descent-based
consensus approaches.

\paragraph{Consensus rates.}
Fraction of tasks achieving full consensus, partial consensus, or
remaining irreconcilable, together with the mean number of rounds
required to reach consensus.

\paragraph{Obstruction analysis.}
Distribution of obstruction classes (type conflicts, semantic
incompatibilities, API contract violations, style disagreements) and
their resolution rates under treaty synthesis.

\paragraph{Treaty synthesis.}
Number of treaties generated and accepted, acceptance rates, and mean
synthesis time.

\paragraph{Timing.}
Per-component latencies (descent, consensus, treaty synthesis) and
total pipeline time, including overhead relative to single-agent
execution.

% ─────────────────────────────────────────────────────────────────────
\subsection{Planned Ablation Study}\label{subsec:ablation}

The ablation study is designed to isolate the effects of ensemble
size, overlap degree, and treaty type.

\paragraph{Number of agents.}
Varying the number of agents would quantify the trade-off between
disagreement probability and ensemble quality.

\paragraph{Overlap degree.}
Adjusting the number of agents per coordinate would measure the
trade-off between consensus rate and conflict detection capability.
The expected optimal balance is 2--3 agents per critical coordinate.

\paragraph{Treaty types.}
Evaluating each treaty type separately (style, type unification, API
standardization, semantic re-querying) would reveal which conflict
categories are most amenable to automated resolution.
...
consensus.  Obstruction classes precisely diagnose disagreement types and
locations, and treaty synthesis automates conflict resolution.  The
ensemble approach, enabled by descent verification, is designed to produce
code that is strictly better than any individual agent's output.
Quantitative validation of consensus rates and quality improvements
