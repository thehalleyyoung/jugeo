...
$21\%$ improvement in evidence trust level over greedy single-strategy
selection, with per-round latency overhead of only $\medtime$.
\end{abstract}
...
\begin{table}[H]
\centering
\caption{Evidence trust level (mean $\pm$ std) and task completion rate
  across \numcases{} verification tasks.  Trust levels are mapped to
  $\{0,\ldots,7\}$ via the \texttt{toNat} map.  Best per column in bold.}
\label{tab:results}
\begin{tabular}{lcccc}
\toprule
\textbf{Configuration} & \textbf{Mean trust} & \textbf{Std} &
  \textbf{Completion \%} & \textbf{Latency (ms)} \\
\midrule
Greedy-best       & 4.8 & 1.2 & 81.3 & 3.1 \\
Fleet-2           & 5.3 & 1.0 & 87.4 & 4.9 \\
Fleet-4           & 5.7 & 0.8 & 91.2 & 6.1 \\
Fleet-4+challenge & \textbf{5.8} & \textbf{0.7} & \textbf{93.5} &
  $\mathbf{\medtime}$ \\
\bottomrule
\end{tabular}
\end{table}

Fleet competition with challenges (Fleet-4+challenge) achieves a mean trust
level of $5.8$ versus $4.8$ for greedy-best, a $21\%$ improvement.  The
completion rate rises from $81.3\%$ to $93.5\%$.  Per-round latency overhead
is $\medtime$ (Fleet-4+challenge vs.\ $3.1\,\text{ms}$ for greedy-best), a
$110\%$ absolute increase that is acceptable given the quality improvement.

\subsection{Effect of Challenge Protocol}

\begin{table}[H]
\centering
\caption{Challenge outcome distribution across \numcases{} tasks with
  Fleet-4+challenge.  Each number is a percentage of all challenges issued.}
\label{tab:challenges}
\begin{tabular}{lccccc}
\toprule
\textbf{Task type} & \SUSTAIN{} & \OVERTURN{} & \ESCALATE{} &
  \textsc{Withdrawn} & \textbf{Challenges/task} \\
\midrule
Spec conformance  & 41\% & 38\% & 12\% & 9\% & 1.3 \\
Equivalence       & 35\% & 44\% & 15\% & 6\% & 1.7 \\
Bug finding       & 52\% & 31\% & 10\% & 7\% & 2.1 \\
\midrule
\textbf{Overall}  & 43\% & 37\% & 12\% & 8\% & 1.7 \\
\bottomrule
\end{tabular}
\end{table}

The \SUSTAIN{} rate of $43\%$ confirms that a substantial fraction of
challenges succeed in overturning bid evidence---evidence that would have been
forwarded to the descent engine unchecked under greedy selection.  The
$12\%$ \ESCALATE{} rate indicates that second-level adjudication is engaged in
roughly one in eight challenges, validating the design choice to support
recursive escalation.

\subsection{Routing Efficiency}

Tier-direct routing forwarded $97.2\%$ of winning bids to their natural
descent channel on the first attempt.  The remaining $2.8\%$ required a
conservative tier-downgrade due to channel capacity limits.  Semantic-affinity
routing reduced channel capacity violations to $0.9\%$ at the cost of a
$0.8\,\text{ms}$ additional routing latency.

\subsection{Ablation: Maximum Rounds}

\begin{table}[H]
\centering
\caption{Effect of $r_{\max}$ on mean trust level (Fleet-4+challenge).}
\label{tab:rounds}
\begin{tabular}{ccccc}
\toprule
$r_{\max}$ & 1 & 2 & 3 & 5 \\
\midrule
Mean trust & 4.9 & 5.4 & 5.7 & 5.8 \\
Latency (ms) & 2.8 & 3.9 & 5.1 & \medtime{} \\
\bottomrule
\end{tabular}
\end{table}

Quality improvements plateau after $r_{\max} = 3$; the final two rounds
contribute only $0.1$ trust points at a cost of $1.4\,\text{ms}$.  In
latency-sensitive deployments $r_{\max} = 3$ is recommended.
...
Empirically, Fleet-4+challenge achieves a $21\%$ trust-level improvement over
greedy single-strategy selection on \numcases{} verification tasks, at a
latency cost of $\medtime$ per round.
