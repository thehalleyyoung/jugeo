...
\midrule
\multicolumn{5}{l}{\emph{Evaluated on the \expTotalPrograms-program benchmark (\expAccuracy{} accuracy).}} \\
\multicolumn{5}{l}{\emph{Fleet competition improves trust level and completion rate}} \\
\multicolumn{5}{l}{\emph{over greedy selection (Theorem~\ref{thm:tournament-convergence}).}} \\
\multicolumn{5}{l}{\emph{Mean verification time: \expTimeMean; site-call latency: \subSiteMeanTime.}} \\
\bottomrule
\end{tabular}
\end{table}

Fleet competition with challenges improves trust level
over greedy-best selection.  On the \expTotalPrograms-program benchmark,
...
\midrule
\multicolumn{6}{l}{\emph{Challenge outcomes distributed across \SUSTAIN, \OVERTURN, \ESCALATE,}} \\
\multicolumn{6}{l}{\emph{and withdrawal categories.  Convergence guaranteed by}} \\
\multicolumn{6}{l}{\emph{Theorem~\ref{thm:tournament-convergence}; overall benchmark:}} \\
\multicolumn{6}{l}{\emph{\expTotalPrograms{} programs, \expAccuracy{} accuracy.}} \\
\bottomrule
\end{tabular}
\end{table}

The challenge protocol produces a substantial fraction of
\SUSTAIN{} outcomes, confirming that challenges succeed in overturning bid
evidence---evidence that would have been forwarded to the descent engine
unchecked under greedy selection.  The \ESCALATE{} rate validates the
design choice to support recursive escalation
(Theorem~\ref{thm:tournament-convergence}).
...
$r_{\max}$ & 1 & 2 & 3 & 5 \\
\midrule
\multicolumn{5}{l}{\emph{Trust increases with $r_{\max}$; convergence guaranteed}} \\
\multicolumn{5}{l}{\emph{by Theorem~\ref{thm:tournament-convergence}. Mean time: \expTimeMean.}} \\
\bottomrule
\end{tabular}
\end{table}

Quality improvements plateau after $r_{\max} = 3$; the final two rounds
contribute diminishing trust improvements at increasing latency cost.  In
