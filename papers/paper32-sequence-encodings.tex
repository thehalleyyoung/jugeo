...
% Mutation operations
\newcommand{\MutApp}{\mathsf{append}}
\newcommand{\MutIns}{\mathsf{insert}}
\newcommand{\MutPop}{\mathsf{pop}}
\newcommand{\MutDel}{\mathsf{del}}
\newcommand{\MutSlice}{\mathsf{slice}}
\newcommand{\MutAssign}{\mathsf{assign}}
\newcommand{\MutOp}{\mathsf{MutOp}}
...
shows a median encoding time of \medtime{} per file and a 97.3\% agreement
...
\MutIns       & 1.4  & 2.3  & 98.7\\
\MutPop       & 1.2  & 1.9  & 98.9\\
\MutSlice     & 2.1  & 3.4  & 97.8\\
\MutAssign    & 0.6  & 0.9  & 99.4\\
Dict insert   & 1.1  & 1.8  & 97.2\\
Dict delete   & 1.3  & 2.0  & 97.0\\
Aliased mut.  & 3.8  & 6.1  & 95.3\\
\midrule
\textbf{Overall} & \textbf{\medtime} & \textbf{2.7} & \textbf{97.3}\\
\bottomrule
\end{tabular}
\end{table}
...
$\MutApp$ is the most common list operation (43\% of cases) and is the
cheapest to encode: its singleton support means no shift universals need to
be unrolled.

\paragraph{Aliasing overhead.}
Aliased mutations incur a 3.2$\times$ overhead relative to non-aliased ones,
...
\paragraph{Agreement.}
The 2.7\% disagreement rate is attributable to two sources: (a) 1.1\% of
cases involve \emph{step-slicing} (\texttt{xs[lo:hi:step]} with $\mathit{step}
\neq 1$), which our current encoding approximates; (b) 1.6\% involve nested
...
\texttt{collection\_heap\_encodings} and achieves a 97.3\% agreement rate on
