...
% ─────────────────────────────────────────────────────────────────────
\begin{abstract}
We present a dependency-analysis study built on \JuGeo's semantic-site
machinery. Traditional import-graph analysis captures syntactic dependencies
between modules but misses semantic dependencies represented by morphisms
between coordinates. We therefore study dependency chains extracted from
semantic sites and report descriptive measurements on
\ppLXIprogramCount{} curated programs. Across this corpus, the sites contain
\ppLXIcoordsMean{} coordinates and \ppLXImorphMean{} morphisms on average,
with mean chain length \ppLXIchainMean{} and mean analysis time
\ppLXItimeMean{}. All \ppLXIpropsOkSum{} recorded propositions were
solver-discharged and no obstructions were observed. We present these results
as JuGeo-backed measurements of the curated benchmark, not as a comparison
against external dependency-analysis tools.
\end{abstract}
...
topology = GrothendieckTopology.discrete()
site = Site(topology=topology, label="deps")

validate = Coordinate("validate_input", kind=CoordinateKind.FUNCTION)
report = Coordinate("generate_report", kind=CoordinateKind.FUNCTION)
shared = Coordinate("shared_state", kind=CoordinateKind.REGION)

site.add_coordinate(validate)
site.add_coordinate(report)
site.add_coordinate(shared)
site.add_morphism(Morphism(
    source=validate,
    target=shared,
    kind=MorphismKind.RESTRICTION,
    label="validate->shared",
))
site.add_morphism(Morphism(
    source=report,
    target=shared,
...
site.add_covering_family(CoveringFamily(
    base=shared,
    members=site.morphisms_to(shared),
    label="shared-cover",
))

print(site.coordinate_count(), site.morphism_count())
print([coord.name for coord in site.global_sections()])
\end{lstlisting}
...
\midrule
\textbf{Total} & \compMorphTotal & 100\% & (all) \\
\bottomrule
\end{tabular}
\end{table}
