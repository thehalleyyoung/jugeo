...
\begin{table}[h!]
\centering
\caption{The eight \BugKind\ obstruction families.}
\label{tab:bugkinds}
\begin{tabular}{lcccc}
\toprule
\textbf{Kind} & \textbf{Generator} & \textbf{Local?} &
\textbf{Base Severity} & \textbf{Example} \\
\midrule
\texttt{TYPE\_ERROR}            & $\bugtype$  & \checkmark & 0.70
  & Wrong-type argument \\
\texttt{LOGIC\_ERROR}           & $\buglogic$ & \checkmark & 0.80
  & Off-by-one in loop \\
\texttt{SCOPE\_VIOLATION}       & $\bugscope$ & \checkmark & 0.75
  & Undefined variable use \\
\texttt{PROTOCOL\_VIOLATION}    & $\bugproto$ & $\times$   & 0.85
  & Sequence-before-connect \\
\texttt{TRUST\_VIOLATION}       & $\bugtrust$ & \checkmark & 0.90
  & Trust-tier downgrade \\
\texttt{RESOURCE\_LEAK}         & $\bugres$   & \checkmark & 0.65
  & File handle not closed \\
\texttt{CONCURRENCY\_HAZARD}    & $\bugconc$  & $\times$   & 0.95
  & Read-write race \\
\texttt{SPECIFICATION\_DEVIATION}& $\bugspec$ & $\times$   & 0.60
