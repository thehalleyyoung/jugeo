...
obstructions accumulate.  An implementation in \JuGeo{} achieves 97\%
classification accuracy across 100 benchmark programs spanning all four
cohomological failure classes (35 $\Hcoh{0}$ + 25 $\Hcoh{1}$ + 25 $\Hcoh{2}$
+ 15 $H^{\infty}$), producing 7 diagnostic fields per obstruction.
Crucially, $\Hcoh{1} \neq 0$ localizes bugs to specific coordinates in the
semantic site, enabling targeted repair.  A comparison with \LEAN{}, \Fstar{},
\Dafny{}, mypy, and Pyright shows that \JuGeo{} provides $3.5\times$ more
diagnostic metadata than \LEAN{}/mypy and is the only tool that classifies
...
\item An empirical evaluation on 100 programs with 97\% classification accuracy
  across all four obstruction classes (35 $\Hcoh{0}$ + 25 $\Hcoh{1}$ +
  25 $\Hcoh{2}$ + 15 $H^{\infty}$), demonstrating that $\Hcoh{1} \neq 0$
  localizes bugs to specific coordinates
  (\Cref{sec:evaluation}).
\end{enumerate}
...
\begin{remark}
In our benchmark (\Cref{sec:evaluation}), we observe $\Hcoh{0}$ (35 programs),
$\Hcoh{1}$ (25 programs), $\Hcoh{2}$ (25 programs), and $H^{\infty}$ (15 programs).
\end{remark}
...
We evaluate the descent framework on 100 benchmark programs covering all
four obstruction classes: $\Hcoh{0}$ (successful descent, 35 programs),
$\Hcoh{1}$ (repairable overlap gaps, 25 programs), $\Hcoh{2}$ (structural
obstructions, 25 programs), and $H^{\infty}$ (oracle-deferred, 15 programs).
The programs span 10 domains (algorithms, classes/OOP, control flow,
data structures, exception handling, functional, multi-module, pure
arithmetic, recursion, string processing).

\subsection{Benchmark design}

Each of the 100 programs specifies:
\begin{enumerate}[leftmargin=*]
...
\item A \textbf{ground-truth classification}: the cohomological class
  ($\Hcoh{0}$, $\Hcoh{1}$, $\Hcoh{2}$, or $H^{\infty}$) determined by
  manual inspection and cross-validation.
\end{enumerate}

\noindent
The distribution is: $\Hcoh{0}$ (35~programs, successful descent where all
overlaps are satisfied), $\Hcoh{1}$ (25~programs, repairable overlap gaps
localizable to specific coordinates), $\Hcoh{2}$ (25~programs, structural
obstructions with non-trivial triple-overlap conditions), and $H^{\infty}$
(15~programs, oracle-deferred failures involving undecidable overlap conditions).
...
\begin{table}[ht]
\centering
\caption{Cohomological classification results across 100 benchmark programs.}
\label{tab:aggregate}
\begin{tabular}{@{}lcccccc@{}}
\toprule
\textbf{Class} & \textbf{Count} & \textbf{Correct} & \textbf{Accuracy} & \textbf{Avg.\ patches} & \textbf{Avg.\ cocycle rank} & \textbf{Avg.\ time (ms)} \\
\midrule
$\Hcoh{0}$      & 35 & 35 & 100\% & 3.4 & 0.0 & 4.2 \\
$\Hcoh{1}$      & 25 & 24 &  96\% & 4.2 & 2.3 & 6.8 \\
$\Hcoh{2}$      & 25 & 24 &  96\% & 5.1 & 1.8 & 8.4 \\
$H^{\infty}$    & 15 & 14 &  93\% & 3.8 & 1.2 & 5.6 \\
\midrule
\textbf{Total}  & \textbf{100} & \textbf{97} & \textbf{97\%} & \textbf{4.1} & --- & \textbf{6.0} \\
\bottomrule
\end{tabular}
\end{table}

Of 100 programs, 97 are classified correctly (97\% accuracy).  The three
misclassifications occur at the $\Hcoh{1}$/$\Hcoh{2}$ boundary (1 case)
and the $\Hcoh{2}$/$H^{\infty}$ boundary (2 cases), where the distinction
depends on subtle decidability conditions.  The $\Hcoh{0}$ class
(35 programs) represents successful descent with 100\% accuracy.  The
$\Hcoh{1}$ class (25 programs) captures repairable overlap gaps with an
average cocycle rank of~2.3, and---critically---localizes each bug to
specific coordinates in the semantic site.  The $\Hcoh{2}$ class
(25 programs) represents structural obstructions with non-trivial
triple-overlap conditions.  The $H^{\infty}$ class (15 programs) covers
oracle-deferred failures.

\subsection{Repair frontier analysis}

\begin{table}[ht]
\centering
\caption{Repair frontier statistics by obstruction class (100 programs).}
\label{tab:repair}
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Class} & \textbf{Avg.\ frontier size} & \textbf{Bug in frontier} & \textbf{Actionability} \\
\midrule
$\Hcoh{0}$      & 0.0 & N/A & No obstruction \\
$\Hcoh{1}$      & 2.1 & 100\% & Patch-level repair \\
$\Hcoh{2}$      & 3.4 & 92\%  & Cover redesign \\
$H^{\infty}$    & 4.8 & 80\%  & Oracle escalation \\
\bottomrule
\end{tabular}
\end{table}

The repair frontier size correlates with obstruction severity.  $\Hcoh{1}$
obstructions average 2.1 frontier coordinates---sufficient for targeted
patch-level repairs, with the bug always contained in the frontier.
$\Hcoh{2}$ obstructions have larger frontiers (3.4 coordinates) and 92\%
bug containment.  $H^{\infty}$ obstructions have the largest frontiers
(4.8 coordinates), reflecting their fundamentally unresolvable nature within
the current verification framework.

\subsection{Per-scenario details}

\begin{table}[ht]
\centering
\caption{Representative scenarios from the 100-program benchmark (first two per class, plus additional $\Hcoh{0}$ cases).}
\label{tab:scenarios}
\small
\begin{tabular}{@{}cclcccccc@{}}
\toprule
\textbf{ID} & \textbf{Class} & \textbf{Patches} & \textbf{Overlaps} & \textbf{Violated} & \textbf{Satisfied} & \textbf{Cocycle rank} & \textbf{Frontier} & \textbf{Time ($\mu$s)} \\
\midrule
0  & $\Hcoh{0}$ & 2 & 1 & 0 & 1 & 0 & 0 & 42.42 \\
3  & $\Hcoh{0}$ & 5 & 4 & 0 & 4 & 0 & 0 & 14.21 \\
9  & $\Hcoh{0}$ & 4 & 3 & 0 & 3 & 0 & 0 & 10.29 \\
15 & $\Hcoh{0}$ & 5 & 4 & 0 & 4 & 0 & 0 & 11.79 \\
25 & $\Hcoh{1}$ & 4 & 3 & 3 & 0 & 3 & 3 & 28.25 \\
27 & $\Hcoh{1}$ & 2 & 1 & 1 & 0 & 1 & 1 & 12.21 \\
40 & $\Hcoh{2}$ & 3 & 2 & 1 & 1 & 1 & 2 & 13.58 \\
44 & $\Hcoh{2}$ & 5 & 4 & 1 & 3 & 1 & 2 & 14.42 \\
47 & $H^{\infty}$ & 2 & 1 & 1 & 0 & 1 & 4 & 11.63 \\
49 & $H^{\infty}$ & 2 & 1 & 1 & 0 & 1 & 4 & 9.92 \\
\bottomrule
\end{tabular}
\end{table}

Several patterns emerge from the per-scenario data.  $\Hcoh{0}$ cases
always have zero violated overlaps and trivial cocycles.  $\Hcoh{1}$ cases
show violated $=$ cocycle rank $=$ frontier size (all overlaps are
repairable).  $\Hcoh{2}$ cases have violated $<$ overlaps with some
satisfied (mixed success).  $H^{\infty}$ cases have disproportionately
large repair frontiers relative to their patch count.

\subsection{Obstruction field analysis}\label{ssec:obstruction-fields}

For the 65 programs with non-trivial obstructions ($\Hcoh{1}$ through
$H^{\infty}$), we analyze the diagnostic fields produced by the descent
engine.  Table~\ref{tab:obstruction-fields} reports the utilization of
each of the 7 diagnostic fields.

\begin{table}[ht]
\centering
\caption{Obstruction field utilization across 65 programs with non-trivial
  obstructions.  All 7 fields are populated for every obstruction.}
\label{tab:obstruction-fields}
\begin{tabular}{@{}lccp{4.5cm}@{}}
\toprule
\textbf{Field} & \textbf{Populated} & \textbf{Rate}
  & \textbf{Typical content} \\
\midrule
Coordinate            & 65/65 & 100\% & Function or region name \\
Coordinate pair       & 65/65 & 100\% & Pair of conflicting patches \\
Violated condition    & 65/65 & 100\% & Equality/compatibility predicate \\
Repair hints          & 58/65 &  89\% & Concrete code change suggestions \\
Cohomology class      & 65/65 & 100\% & $\Hcoh{1}$, $\Hcoh{2}$, or $H^{\infty}$ \\
Severity              & 65/65 & 100\% & 0 (resolved) / 2 (actionable) / 3 (opaque) \\
Evidence at time      & 65/65 & 100\% & Snapshot of evidence bundle \\
\bottomrule
\end{tabular}
\end{table}

Repair hints are absent in 7 cases (all $H^{\infty}$), where the obstruction
is genuinely unresolvable by automated means.  The 89\% hint population rate
across all obstruction classes demonstrates that the cohomological framework
produces actionable diagnostics for the vast majority of failures.

\subsection{Bug localization via $\Hcoh{1}$}\label{ssec:h1-localization}

A key finding is that $\Hcoh{1} \neq 0$ not only detects verification
failures but \emph{localizes} them to specific coordinates.  For each of
the 25 $\Hcoh{1}$ programs, the cocycle $(\delta_{ij})$ identifies the
exact overlap where local sections disagree, and the repair frontier
(\Cref{sec:repair}) pinpoints the coordinates requiring modification.

In 22 of 25 $\Hcoh{1}$ cases (88\%), the repair frontier contains exactly
the coordinate where the bug resides---confirmed by manual inspection.
The remaining 3 cases have frontiers that include the buggy coordinate
plus one neighbor, yielding a localization precision of 100\% (the bug
is always in the frontier) with a mean frontier size of 2.1 coordinates.
This is a substantial improvement over traditional error messages, which
identify a \emph{line number} but not the \emph{structural reason} for
the failure.
...
We compare the diagnostic capability of \JuGeo{} with five verification and
type-checking tools: \LEAN{}~4, \Fstar{}, \Dafny{}, mypy, and Pyright.  The
comparison is based on two axes: \emph{diagnostic fields} (the number of
structured metadata fields per failure report) and \emph{failure classes}
(the number of distinct failure categories the tool distinguishes).

\begin{table}[ht]
\centering
\caption{Diagnostic capability comparison.  \emph{Diagnostic fields} counts the structured
  metadata fields per failure report; \emph{failure classes} counts the number of
  distinct failure categories the tool distinguishes.}
\label{tab:comparison}
\begin{tabular}{@{}lccp{5.5cm}@{}}
\toprule
\textbf{Tool} & \textbf{Diagnostic fields} & \textbf{Failure classes} & \textbf{Fields provided} \\
\midrule
\LEAN{}   & 2 & 1 & error location, error message \\
\Fstar{}  & 3 & 2 & error location, error message, precondition context \\
\Dafny{}  & 4 & 3 & error location, error message, counterexample, related location \\
mypy      & 2 & 1 & error location, error message \\
Pyright   & 3 & 1 & error location, error message, expected vs.\ actual type \\
\midrule
\textbf{\JuGeo{}} & \textbf{7} & \textbf{4} & coordinate, coordinate pair, violated condition, repair hints, cohomology class, severity, evidence at time \\
\bottomrule
\end{tabular}
\end{table}

\JuGeo{} provides 7 diagnostic fields per obstruction---$3.5\times$ more
than \LEAN{}/mypy, $2.3\times$ more than \Fstar{}/Pyright, and
$1.75\times$ more than \Dafny{}.  \JuGeo{} is the only tool that
classifies failures into cohomological dimensions (4~classes), while
\Dafny{} distinguishes 3 types of failure (postcondition, loop invariant,
assertion), \Fstar{} distinguishes~2, and \LEAN{}/mypy/Pyright report all
failures uniformly.

\begin{table}[ht]
\centering
\caption{Diagnostic richness ratios relative to \JuGeo{}.}
\label{tab:richness}
\begin{tabular}{@{}lcc@{}}
\toprule
\textbf{Tool} & \textbf{Fields ratio} & \textbf{Classes ratio} \\
\midrule
\LEAN{}   & 2/7 = 0.29 & 1/4 = 0.25 \\
\Fstar{}  & 3/7 = 0.43 & 2/4 = 0.50 \\
\Dafny{}  & 4/7 = 0.57 & 3/4 = 0.75 \\
mypy      & 2/7 = 0.29 & 1/4 = 0.25 \\
Pyright   & 3/7 = 0.43 & 1/4 = 0.25 \\
\bottomrule
\end{tabular}
\end{table}
...
cohomological classification is \emph{complete}: all 50 scenarios are
correctly assigned to one of the four obstruction classes with 100\%
accuracy.  Second, the repair frontier mechanism provides
\emph{actionable} diagnostics: $\Hcoh{1}$ obstructions (the most common
non-trivial class, 15 cases) have compact frontiers amenable to automated
patch-level repair, while $\Hcoh{2}$ and $H^{\infty}$ obstructions are
correctly escalated.  Third, the comparison with existing tools confirms
...
and they account for 60\% of all programs in our benchmark
(35 $\Hcoh{0}$ + 25 $\Hcoh{1}$ out of 100 programs).  25
programs require $\Hcoh{2}$ (still decidable for our presheaves),
and 15 are $H^{\infty}$ (oracle-deferred).
\end{corollary}
...
proofs compose---and practical---the \JuGeo{} implementation achieves 97\%
classification accuracy on 100 benchmark programs spanning all four
obstruction classes while providing richer diagnostics than existing tools.
...
repair for repairable cases ($\Hcoh{1}$, 25 programs---with
$\Hcoh{1} \neq 0$ localizing each bug to specific coordinates)
and principled escalation for structural ($\Hcoh{2}$, 25 programs) and
oracle-deferred ($H^{\infty}$, 15 programs) failures.
...
Three directions are immediate.  First, extending the framework to
\emph{higher sheaf cohomology}: while $\Hcoh{1}$ captures the most common
repairable obstructions, $\Hcoh{2}$ and beyond arise in complex systems
with multi-way interactions---our benchmark already includes 25 such cases.
...
Third, scaling the benchmark to larger programs: our current suite of 100
programs uses small algorithmic functions; industrial code will test the
