...
\midrule
\multicolumn{4}{l}{\emph{Minimization substantially reduces model size across all domains.}} \\
\multicolumn{4}{l}{\emph{Minimality guaranteed by Theorem~\ref{thm:cm-completeness};}} \\
\multicolumn{4}{l}{\emph{overall benchmark: \expTotalPrograms{} programs, \expAccuracy{} accuracy.}} \\
\bottomrule
\end{tabular}
...
\midrule
\multicolumn{3}{l}{\emph{Total pipeline latency: sub-millisecond per countermodel}} \\
\multicolumn{3}{l}{\emph{(\subSiteMeanTime{} mean site-call latency). Minimization dominates}} \\
\multicolumn{3}{l}{\emph{(one Z3 query per variable); other stages are pure Python.}} \\
\midrule
\textbf{Total} & \medtime & \\
\bottomrule
\end{tabular}
...
On the \expTotalPrograms-program benchmark, the explanation engine
correctly classifies failure classes, with correctness guaranteed by
Theorem~\ref{thm:cm-completeness}.  All \numcases{}
...
Across the \numcases{} countermodels, the normalizer identifies
distinct failure patterns by canonical hash, substantially reducing the unique
cases to be explained and stored.  The content-addressed store
...
on \numcases{} verification cases show substantial model-size reduction,
sub-millisecond end-to-end latency (\subSiteMeanTime{} mean), and
correctness guaranteed by the Minimality Theorem.
