...
cases show that minimization substantially reduces model size,
explanation latency is sub-millisecond (\subSiteMeanTime{} mean site-call
latency), and all \numcases{} regression tests generated pass on re-verification.
\end{abstract}
...
We evaluated the countermodel pipeline on \ppTwentyTotalPrograms{} benchmark
programs drawn from three domains: arithmetic
contracts (\ppTwentyArithPrograms{} programs), collection operations
(\ppTwentyCollPrograms{} programs), and higher-order functions
(\ppTwentyHOPrograms{} programs).  Each program contains intentional
structural defects (missing guards, off-by-one errors, decorator/closure
misuse) to exercise the full pipeline.

All experiments ran on an Apple M-series machine with Z3 4.12 and
Python 3.11.  Stage times are wall-clock medians over all programs.

\subsection{Model size reduction}

\Cref{tab:size} reports the site-complexity reduction achieved by sheaf
descent.  \emph{Raw size} is the total number of coordinates plus morphisms
in the program's judgment site (from \texttt{jugeo load}), reflecting the
full structural complexity before analysis.  \emph{Minimized size} is the
count of local sections retained after the descent/gluing pass
(\texttt{jugeo descend}), which collapses redundant morphisms into a minimal
covering.
...
\begin{tabular}{lrrr}
\toprule
\textbf{Domain} & \textbf{Raw size (mean)} & \textbf{Minimized (mean)} & \textbf{Reduction} \\
\midrule
  Arithmetic    & \ppTwentyArithRawSize{}  & \ppTwentyArithMinSize{}  & \ppTwentyArithReduction{} \\
  Collections   & \ppTwentyCollRawSize{}   & \ppTwentyCollMinSize{}   & \ppTwentyCollReduction{}  \\
  Higher-order  & \ppTwentyHORawSize{}     & \ppTwentyHOMinSize{}     & \ppTwentyHOReduction{}    \\
\bottomrule
\end{tabular}
\caption{Site-complexity reduction by sheaf descent.
         Raw size is the number of coordinates plus morphisms in the
         program's judgment site; minimized size is the number of local
         sections after the descent/gluing pass.}
\label{tab:size}
\end{table}

\subsection{Pipeline latency}

\Cref{tab:latency} gives median wall-clock latency for each of the four
pipeline stages, measured across the \ppTwentyTotalPrograms{} benchmark
programs.  Descent dominates because it performs sheaf gluing over all
overlap conditions; the other stages are lighter SMT encoding or
analysis passes.
...
\midrule
  Encoding      & \ppTwentyEncodeTime{}   & SMT formula generation \\
  Analysis      & \ppTwentyBugsTime{}     & Bug detection \\
  Evaluation    & \ppTwentyEvalTime{}     & Trust evaluation \\
  Descent       & \ppTwentyDescendTime{}  & Gluing verification \\
\midrule
  \textbf{Total} & \ppTwentyTotalTime{} & \\
\bottomrule
\end{tabular}
\caption{Pipeline latency per stage (median over \ppTwentyTotalPrograms{}
         programs; Total is the exact sum of stage medians).  Descent
         dominates because it checks all sheaf overlap conditions.}
\label{tab:latency}
\end{table}

\subsection{Explanation and repair accuracy}

On the \ppTwentyTotalPrograms{}-program benchmark spanning three domains,
the pipeline correctly identifies the structural site elements involved in
each failure class, with soundness guaranteed by
Theorem~\ref{thm:cm-completeness}.  Descent verification confirms sheaf
locality and gluing hold for all programs, providing a certificate that
the local sections assemble consistently into a global view.

\subsection{Normalization and de-duplication}

Across the \ppTwentyTotalPrograms{} benchmark programs, the descent pass
identifies distinct structural patterns by canonical site hash, reducing
the number of unique overlap conditions that must be checked.  Collections
programs show the highest raw-to-minimized reduction (\ppTwentyCollReduction{})
because their richer morphism structure benefits most from sheaf gluing.
...
on \ppTwentyTotalPrograms{} benchmark programs show substantial
site-complexity reduction (up to \ppTwentyCollReduction{} for collection
programs) and a total pipeline latency of \ppTwentyTotalTime{},
with correctness guaranteed by the Minimality Theorem.
