...
\newcommand{\ShapeErr}{\ensuremath{\mathsf{ShapeError}}}        % shape-error event
...
\begin{tabular}{@{}lrrrl@{}}
\toprule
\textbf{Category} & \textbf{Programs} & \textbf{Mean coords} &
\textbf{Mean time} & \textbf{Result} \\
\midrule
Tiny ($\leq$10 lines)   & \compTinyCount   & \compTinyMeanCoords   & \compTinyMeanTime   & \textcolor{jugeoGreen}{ALL SAFE} \\
Small (11--25 lines)      & \compSmallCount  & \compSmallMeanCoords  & \compSmallMeanTime  & \textcolor{jugeoGreen}{ALL SAFE} \\
Medium (26--60 lines)     & \compMediumCount & \compMediumMeanCoords & \compMediumMeanTime & \textcolor{jugeoGreen}{ALL SAFE} \\
Large ($>$60 lines)       & \compLargeCount  & \compLargeMeanCoords  & \compLargeMeanTime  & \textcolor{jugeoGreen}{ALL SAFE} \\
\midrule
\textbf{Total}           & \compTotalPrograms & \compCoordsMean & \compTimeMean & \textcolor{jugeoGreen}{\compOverallAccuracy} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Quantifier discipline usage across the benchmark.}
\label{tab:qdiscipline}
\begin{tabular}{@{}ll@{}}
\toprule
\textbf{Discipline} & \textbf{Usage} \\
\midrule
\texttt{alwaysQF}    & Dominant (shape properties are quantifier-free) \\
\texttt{instantiate} & Rare (element-wise assertions only) \\
\texttt{skolem}      & Rare (existential witnesses) \\
\texttt{inlineQuant} & Very rare (full quantifier inlining) \\
\bottomrule
\end{tabular}
\end{table}
