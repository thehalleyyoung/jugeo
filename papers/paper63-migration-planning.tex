...
% ─────────────────────────────────────────────────────────────────────
\begin{abstract}
We present a categorical framework for planning and executing code
migrations using \emph{change-of-site functors} in \JuGeo.  When a program
evolves---through refactoring, API upgrades, or framework migrations---the
semantic site changes from $\SiteOld$ to $\SiteNew$.  The change-of-site
functor $\FunctorF : \SiteOld \to \SiteNew$ maps coordinates and morphisms
of the old site to the new one, inducing a pullback $\Pullback$ on
presheaves that transfers verified judgments from the new site back to the
old one, and a pushforward $\Pushforward$ that propagates judgments forward.
Using the \ChangeOfSite, \RunDescent, and \ReplayGlue\ API methods, we
compute the migration plan: which judgments transfer directly, which require
re-verification, and which face descent obstructions in the new site.
Experiments on \compTotalPrograms\ programs show that the change-of-site
functor preserves \compTrustSolverdischargedPct\ of solver-discharged
judgments, with mean migration analysis time \compTimeMean.  We prove that
the functor preserves descent data when $\Hcoh{1}$ vanishes, and formalize
this in Lean~4.  The framework reduces migration effort by identifying
the minimal re-verification set and providing automated repair suggestions
via \RepairSem.
\end{abstract}
...
functor = Site.change_of_site("v1/src/", "v2/src/")
plan = functor.migration_plan()
print(f"Preserved: {len(plan.preserved)}")
print(f"Modified: {len(plan.modified)} (re-verify)")
print(f"Obstructed: {len(plan.obstructed)} (need repair)")
print(f"Cost: {plan.cost:.1f}, Risk: {plan.risk:.2%}")

for coord, obstruction in plan.obstructed:
    repair = functor.site_new.repair_semantics(
        coord=functor.map_coord(coord),
        obstruction=obstruction)
    status = "repaired" if repair.success else "manual"
