...
invariant.  Empirically, the engine discovers $3.1\times$ more useful
invariants per unit time than manual triage, with novelty-guided search
accounting for the majority of the improvement.
\end{abstract}
...
\textbf{Strategy} & \textbf{Found} & \textbf{Useful} & \textbf{Provable}
  & \textbf{Time (s)} \\
\midrule
Manual triage (baseline) & 18.4 & 12.1 & 9.8 & 60 \\
\alg{RANDOM}             & 41.2 &  9.3 & 6.1 & 60 \\
\alg{GREEDY}             & 52.7 & 28.4 & 21.0 & 60 \\
\alg{BEAM\_SEARCH} ($k=8$) & 49.1 & 31.2 & 23.3 & 60 \\
\alg{OBSTRUCTION\_MINING}  & 38.8 & 35.6 & 27.1 & 60 \\
\alg{HYBRID}             & 57.2 & \textbf{37.5} & \textbf{29.4} & 60 \\
\alg{EXHAUSTIVE}         & \textbf{91.4} & 34.8 & 25.1 & 60 \\
\bottomrule
\end{tabularx}
\end{table}
...
\midrule
\alg{SEMANTIC}    & 84 & 312 & 187 & 104 \\
\alg{STRUCTURAL}  & 84 & 241 & 201 & 130 \\
\alg{TOPOLOGICAL} & 84 & 178 & 162 & 118 \\
\alg{HYBRID}      & 84 & \textbf{497} & \textbf{349} & \textbf{241} \\
\bottomrule
\end{tabular}
\end{table}

\alg{STRUCTURAL} transport achieves the highest precision (proved /
transported = 54\%), confirming that structural analogies preserve
...
\addplot coordinates {
  (Bound,34) (Monoton.,28) (Complete.,16) (Optimal.,11)
  (Approx.,7) (Impossib.,4)
};
\end{axis}
\end{tikzpicture}
...
Bound-type invariants dominate (34\%), reflecting their prevalence in
...
Wall time scales sub-linearly in codebase size (180\,KLOC is
$18\times$ larger but takes only $9.5\times$ longer), indicating
that the federation's workload partitioning is effective.
...
Empirically, the engine discovers $3.1\times$ more useful invariants
per minute than manual triage, and analogy transport successfully
transfers 70\% of accepted candidates to structurally similar programs.
