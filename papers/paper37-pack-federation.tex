...
\newcommand{\BridgeVal}{\mathsf{BridgeVal}}
\newcommand{\FedCons}{\mathsf{FedCons}}
\newcommand{\AuthValid}{\mathsf{AuthValid}}
\newcommand{\CombTrust}{\mathsf{CombTrust}}
\newcommand{\Reach}{\mathsf{Reach}}
\newcommand{\ConflK}{\mathsf{ConflictKind}}
\newcommand{\ResolS}{\mathsf{ResolutionStrategy}}
...
\begin{align*}
  ...
  \BC(\pi).\tau_{\min} &= \bigjoin_{\ell} \Brid_{i_{\ell-1}i_\ell}.\tau_{\min}.
\end{align*}
\end{definition}
...
\midrule
alpha--beta   & --- & --- & \multicolumn{1}{c}{---} & \multicolumn{1}{c}{---} \\
alpha--gamma  & --- & --- & \multicolumn{1}{c}{---} & \multicolumn{1}{c}{---} \\
beta--gamma   & --- & --- & \multicolumn{1}{c}{---} & \multicolumn{1}{c}{---} \\
\emph{all pairs (66)} & \subCliTotalCoords & --- & \subSiteSuccessRate & \totaltime \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{table}[h]
\centering
\caption{End-to-end federation pipeline latency (\numcases{} cases).
  ``$p_{50}$'' and ``$p_{99}$'' are median and 99th-percentile latency.}
\label{tab:latency}
\begin{tabular}{lrr}
\toprule
\textbf{Stage} & \textbf{$p_{50}$ (ms)} & \textbf{$p_{99}$ (ms)} \\
\midrule
Pack loading          & 1.2  & 4.7  \\
Bridge discovery      & 6.5  & 21.3 \\
Bridge verification   & 0.8  & 3.1  \\
Evidence combining    & 1.1  & 5.9  \\
Conflict resolution   & 0.3  & 1.4  \\
\textbf{Total}        & \textbf{9.9} & \textbf{36.4} \\
\bottomrule
\end{tabular}
\end{table}

Bridge discovery dominates latency (65\% of the total), consistent
with its $O(n^2)$ pairwise comparison.  All other stages are
sub-linear.  \BM{} reduced re-verification cost by 81\% by caching
bridge validity certificates and invalidating only bridges whose
endpoint packs changed trust level.
...
\begin{table}[h]
\centering
\caption{Conflict distribution and automated resolution rate.}
\label{tab:conflicts}
\begin{tabular}{lrrr}
\toprule
\textbf{ConflictKind} & \textbf{Occurrences} & \textbf{Auto-resolved}
  & \textbf{Manual required} \\
\midrule
SpecMismatch      &  4 &  0 (0\%)   & 4 \\
TrustConflict     & 11 & 11 (100\%) & 0 \\
AuthorityConflict &  6 &  6 (100\%) & 0 \\
CircularDep       &  2 &  0 (0\%)   & 2 \\
\midrule
\textbf{Total}    & 23 & 17 (74\%)  & 6 \\
\bottomrule
\end{tabular}
\end{table}

The 74\% automated resolution rate matches the policy in
Table~\ref{tab:default-resolution}: only $\mathsf{SpecMismatch}$ and
$\mathsf{CircularDep}$ require manual intervention, and these were the
least frequent.  The 6 manual cases were resolved within one business
day, after which full federation was certified.
