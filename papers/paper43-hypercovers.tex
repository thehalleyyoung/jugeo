...
ordinary Čech cohomology $\Hcoh{1}_{\mathrm{Čech}}$ on any
paracompact semantic site, closing the gap between the two formalisms.

We evaluate on a benchmark of 47 Python projects with nesting depth up
to~6, demonstrating that hypercovers reduce the number of missed
inter-module obligation edges by $84\%$ compared to flat Čech covers
at an overhead of $\approx 2.1\times$ in construction time.
\end{abstract}
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item \textbf{Comparison theorem} (\S\ref{sec:comparison}):
    $\Hcoh{1}_{\HC} \cong \Hcoh{1}_{\mathrm{Čech}}$ for paracompact
    sites.
\end{enumerate}
...
\begin{remark}
  ...
  hypercover} $\HC^{\mathrm{Č}}_\bullet$ in which $\HC^{\mathrm{Č}}_n
...
\begin{proof}
  ...
  $\HC^{\mathrm{Č}}_\bullet$ (Remark after Definition~\ref{def:hc}).
  The cocycles of the Čech complex embed into
  $\mathrm{Tot}(\mathcal{F}(\HC^{\mathrm{Č}}_\bullet))$ in degree~1,
  so there is a natural map
  $\Hcoh{1}_{\mathrm{Č}} \to \HCech{1}_{\HC}$.
  ...
  \[
    E_2^{p,q} = \Hcoh{p}_{\mathrm{Č}}(X, \mathcal{H}^q(\mathcal{F}))
    \;\Rightarrow\;
  ...
  $\Hcoh{1}_{\mathrm{Č}}(X, \mathcal{F}) \cong \Hcoh{1}(X, \mathcal{F})$.
  ...
  Steps~1 and~2 show that both $\Hcoh{1}_{\mathrm{Č}}$ and
...
\begin{corollary}[Obstruction equivalence]\label{cor:obstr}
  An obstruction class in $\Hcoh{1}_{\mathrm{Č}}(X, \mathcal{F})$
...
We evaluate on 47 Python open-source projects selected from PyPI with
nesting depth~$d \in \{2, 3, 4, 5, 6\}$ (roughly 9--10 projects per
depth level).  Each project is run through the \JuGeo{} pipeline with
two configurations:
\begin{enumerate}
...
\begin{table}[H]
\centering
\caption{Flat Čech vs.\ hypercover on the 47-project benchmark.
  Each cell shows mean $\pm$ std.\ dev.}\label{tab:results}
\begin{tabular}{lrrr}
\toprule
\textbf{Method} & \textbf{Missed edges} & \textbf{Constr.\ (ms)} &
  \textbf{Verif.\ (ms)} \\
\midrule
...
Hypercovers reduce missed edges by $84\%$ on average (from 31.4 to 5.0
per project) at a construction overhead of $2.1\times$ and a
verification overhead of only $1.15\times$.  The gap between
...
\begin{tabular}{crrr}
\toprule
$d$ & \textbf{Projects} & \textbf{Flat Čech} & \textbf{Hypercover} \\
\midrule
2 & 10 & $4.2$  & $0.8$ \\
3 & 9  & $14.1$ & $1.9$ \\
4 & 10 & $28.7$ & $3.4$ \\
...
Of the 47 projects, the synthesis pipeline selected:
\KindCech{} in 18 cases (shallow hierarchies),
\KindTrunc{} in 22 cases (medium depth, $d \in \{3,4,5\}$),
\KindAug{} in 5 cases (deeply nested with explicit package roots),
and \KindSplit{} in 2 cases (highly regular grid packages).
