...
to~6, demonstrating that hypercovers substantially reduce the number of missed
...
\textbf{Method} & \textbf{Missed edges} & \textbf{Constr.\ (ms)} &
  \textbf{Verif.\ (ms)} \\
\midrule
Flat Čech  & \emph{higher}  & \subHypercovertreatySmallTime & --- \\
Hypercover & \emph{lower}   & \subHypercovertreatySmallTime & --- \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
2 & 10 & \emph{low}  & \emph{low} \\
3 & 9  & \emph{moderate} & \emph{low} \\
4 & 10 & \emph{moderate} & \emph{low} \\
5 & 9  & \emph{high} & \emph{moderate} \\
6 & 9  & \emph{high} & \emph{moderate} \\
\bottomrule
\end{tabular}
\end{table}
