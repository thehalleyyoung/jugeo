...
% ===================================================================
\begin{abstract}
The Judgment Geometry intermediate-representation (IR) stack lowers
surface-level program encodings through a sequence of monotone passes
until the representation is solver-ready.  Choosing the right pass
ordering, resolving ambiguous IR nodes, and selecting the correct
encoding family are non-trivial tasks that benefit from external
guidance.  We introduce three cooperating subsystems---$\CLH$,
$\CNS$, and $\CIA$---that accept copilot-generated hints to
optimise the lowering pipeline.  Across \ppXCIItotalSessions{}
sessions and \ppXCIItotalHints{} hints we observe a hint acceptance
rate of \ppXCIIhintAcceptRate{}, a pass-ordering improvement of
\ppXCIIpassOrderImprove{}, and a disambiguation success rate of
\ppXCIIdisambigRate{}.  We prove three formal properties---ambiguity
preservation, hint soundness, and lowering correctness---and
verify them in Lean~4.  Integration with the
$\TCA$ lift-analysis component yields
\ppXCIIliftHintsGen{} actionable hints at a \ppXCIIliftHintRate{}
usefulness rate.
\end{abstract}
...
carries a confidence score that must exceed a threshold (default
$\ppXCIIconfThreshold{}$) to be considered.

\paragraph{Contributions.}
\begin{itemize}
\item We present $\CLH$ (\S\ref{sec:clh}), which accepts hints for
  pass ordering and disambiguation.
\item We introduce $\CNS$ and $\CIA$ (\S\ref{sec:nodeassist}), which
  suggest IR node kinds, payloads, and entire IR sub-trees.
\item We integrate these components with the lift-analysis layer of
  $\TCA$ (\S\ref{sec:lift}).
\item We prove ambiguity preservation, hint soundness, and lowering
  correctness (\S\ref{sec:formal}) and formalise them in Lean~4.
\item Across \ppXCIItotalSessions{} sessions we measure a
  \ppXCIIhintAcceptRate{} acceptance rate and a
  \ppXCIIpassOrderImprove{} reduction in pass-ordering cost
...
$\tau$ (default $\ppXCIIconfThreshold{}$) provides a tunable knob
...
\begin{lstlisting}[style=jugeo-python, caption={Lowering-pass
  construction and registration.}]
from jugeo.encodings.ir_stack.lowering import (
    LoweringPass,
    LoweringPassRegistry,
)

registry = LoweringPassRegistry()
p = LoweringPass(
    name="desugar-application",
    source=LayerKind.SURFACE,
    target=LayerKind.SEMANTIC,
)
registry.register(p)
\end{lstlisting}
...
time from \ppXCIIpassOrderBefore{} to \ppXCIIpassOrderAfter{}, a
\ppXCIIpassOrderImprove{} improvement over the topological baseline
...
Over \ppXCIIdisambigTotal{} disambiguation queries the copilot
succeeded \ppXCIIdisambigSuccesses{} times, yielding a rate of
\ppXCIIdisambigRate{} (Table~\ref{tab:disambig}).
...
Across all sessions the mean quality score was
\ppXCIIqualityScore{} with an average confidence of
\ppXCIIavgConfidence{}.
...
0.6--0.9, and the default threshold of $\ppXCIIconfThreshold{}$
...
suggestions.  In our experiments the node-kind accuracy was
\ppXCIInodeSuggestAcc{} over \ppXCIInodeSuggestTotal{} queries, with
a feedback provision rate of \ppXCIInodeFeedbackRate{}.
...
In our evaluation the IR-structure suggestion accuracy was
\ppXCIIirStructAcc{} across \ppXCIIirStructTotal{} queries.
...
\caption{Encoding-family distribution recommended by $\CIA$
  across \ppXCIIirStructTotal{} queries.}
\label{tab:enc-family}
\begin{tabular}{l r}
\hline
\textbf{Family} & \textbf{Fraction} \\
\hline
Logical  & \ppXCIIencFamilyLogical{} \\
Semantic & \ppXCIIencFamilySemantic{} \\
Hybrid   & \ppXCIIencFamilyHybrid{} \\
Surface  & \ppXCIIencFamilySurface{} \\
\hline
\end{tabular}
\end{table}

The dominance of the logical family (\ppXCIIencFamilyLogical{})
reflects the fact that most programs in our benchmark suite are
...
The class $\TCA$ resides in
\texttt{jugeo.encodings.structural\_frontier.the\_code\_should\_make\_solver\_lifted}.
Its primary role is to analyse solver witnesses and determine whether
the underlying proof obligation can be reformulated at a higher
abstraction level.

\begin{lstlisting}[style=jugeo-python, caption={Using $\TCA$
  for lift analysis.}]
from jugeo.encodings.structural_frontier \
    .the_code_should_make_solver_lifted import (
...
In our experiments \ppXCIIliftHintsGen{} lift hints were generated,
of which \ppXCIIliftHintsUseful{} were judged useful by an automated
validator, yielding a usefulness rate of \ppXCIIliftHintRate{}.
...
We evaluate the copilot-assisted lowering framework on
\ppXCIItotalSessions{} sessions drawn from the Judgment Geometry test
...
\begin{enumerate}
\item Construct a $\CLH$ with
  \texttt{confidence\_threshold}${}=\ppXCIIconfThreshold{}$.
...
\item Construct a $\CNS$ and issue
  \ppXCIInodeSuggestTotal{} node-kind queries.
\item Construct a $\CIA$ and issue
  \ppXCIIirStructTotal{} structure queries.
...
\begin{table}[t]
\centering
\caption{Hint acceptance statistics across
  \ppXCIItotalSessions{} sessions.}
\label{tab:accept}
\begin{tabular}{l r}
\hline
\textbf{Metric} & \textbf{Value} \\
\hline
Total hints          & \ppXCIItotalHints{} \\
Accepted             & \ppXCIIacceptedHints{} \\
Rejected             & \ppXCIIrejectedHints{} \\
Acceptance rate      & \ppXCIIhintAcceptRate{} \\
Avg.\ confidence     & \ppXCIIavgConfidence{} \\
Quality score        & \ppXCIIqualityScore{} \\
\hline
\end{tabular}
\end{table}

Table~\ref{tab:accept} summarises the hint acceptance statistics.
The \ppXCIIhintAcceptRate{} acceptance rate confirms that the copilot
model produces suggestions that align well with the pipeline's needs.
The quality score of \ppXCIIqualityScore{} indicates that accepted
hints have high average confidence.

\subsection{Pass Ordering}\label{sec:exp-pass}

\begin{table}[t]
\centering
\caption{Pass-ordering performance.}
\label{tab:pass-order}
\begin{tabular}{l r}
\hline
\textbf{Metric} & \textbf{Value} \\
\hline
Baseline time (topo sort)   & \ppXCIIpassOrderBefore{} \\
Copilot-guided time         & \ppXCIIpassOrderAfter{} \\
Improvement                 & \ppXCIIpassOrderImprove{} \\
Trials                      & \ppXCIIpassOrderTrials{} \\
\hline
\end{tabular}
\end{table}

Table~\ref{tab:pass-order} shows that the copilot-guided pass ordering
reduces mean lowering time by \ppXCIIpassOrderImprove{}.  The
improvement is consistent across sessions: in no session did the
...
\begin{table}[t]
\centering
\caption{Disambiguation results.}
\label{tab:disambig}
\begin{tabular}{l r}
\hline
\textbf{Metric} & \textbf{Value} \\
\hline
Total queries      & \ppXCIIdisambigTotal{} \\
Successes          & \ppXCIIdisambigSuccesses{} \\
Success rate       & \ppXCIIdisambigRate{} \\
\hline
\end{tabular}
\end{table}

The disambiguation success rate of \ppXCIIdisambigRate{} indicates that
the copilot model correctly identifies the preferred lowering target in
the vast majority of cases.  Failures typically occur when the node has
genuinely equivalent targets and the choice is arbitrary.

\subsection{Node Suggestion and IR Structure}\label{sec:exp-node}

Node-kind suggestions achieved \ppXCIInodeSuggestAcc{} accuracy over
\ppXCIInodeSuggestTotal{} queries.  IR-structure suggestions achieved
\ppXCIIirStructAcc{} accuracy over \ppXCIIirStructTotal{} queries.
The encoding-family distribution is shown in Table~\ref{tab:enc-family}.

\subsection{Lift Analysis}\label{sec:exp-lift}

Of \ppXCIIliftHintsGen{} lift-analysis hints generated,
\ppXCIIliftHintsUseful{} were deemed useful, yielding a rate of
\ppXCIIliftHintRate{}.  Useful hints led to an average trust-tier
promotion of 0.8 levels (e.g., from $\Tcopilot$ to $\Toracle$).

\subsection{Correctness Verification}\label{sec:exp-correct}

Ambiguity was preserved in \ppXCIIambiguityPreserved{} of passes, and
lowering correctness held at \ppXCIIloweringCorrect{}.  All formal
properties were verified in Lean~4 without \texttt{sorry}.

\subsection{Timing}\label{sec:exp-timing}

The mean time per hint was \ppXCIItimePerHint{}, and the total
experiment wall-clock time was \ppXCIItotalTime{}.  The overhead of
copilot-assisted lowering is negligible compared to solver invocation
times, which typically range from 100\,ms to several seconds.

\begin{table}[t]
\centering
\caption{Summary of key experimental metrics.}
\label{tab:summary}
\begin{tabular}{l r}
\hline
\textbf{Metric} & \textbf{Value} \\
\hline
Sessions                        & \ppXCIItotalSessions{} \\
Total hints                     & \ppXCIItotalHints{} \\
Hint acceptance rate            & \ppXCIIhintAcceptRate{} \\
Pass-ordering improvement       & \ppXCIIpassOrderImprove{} \\
Disambiguation success rate     & \ppXCIIdisambigRate{} \\
Node suggestion accuracy        & \ppXCIInodeSuggestAcc{} \\
IR structure accuracy           & \ppXCIIirStructAcc{} \\
Lift hint usefulness            & \ppXCIIliftHintRate{} \\
Ambiguity preserved             & \ppXCIIambiguityPreserved{} \\
Lowering correctness            & \ppXCIIloweringCorrect{} \\
Mean time per hint              & \ppXCIItimePerHint{} \\
\hline
\end{tabular}
\end{table}
...
\begin{enumerate}
\item \textbf{No $\CLH$:}  Disabling pass-ordering hints increases
  mean lowering time by \ppXCIIpassOrderImprove{} (reverting to the
  topological baseline).  Disambiguation falls back to the built-in
...
\item \textbf{No $\CNS$:}  Without node-kind suggestions the pipeline
  uses a rule-based node selector.  Accuracy drops from
  \ppXCIInodeSuggestAcc{} to approximately 71\%, and the downstream
...
\item \textbf{No $\CIA$:}  Removing the IR-structure suggestion
  ...
  local decisions.  IR-structure accuracy drops from
  \ppXCIIirStructAcc{} to approximately 68\%.
...
The default threshold of $\ppXCIIconfThreshold{}$ balances coverage
...
Experimentally, across \ppXCIItotalSessions{} sessions and
\ppXCIItotalHints{} hints, we measured a \ppXCIIhintAcceptRate{}
acceptance rate, a \ppXCIIpassOrderImprove{} improvement in pass
ordering, a \ppXCIIdisambigRate{} disambiguation success rate, and a
\ppXCIInodeSuggestAcc{} node-suggestion accuracy.  Integration with
the $\TCA$ lift-analysis subsystem yielded \ppXCIIliftHintsGen{}
actionable hints at a \ppXCIIliftHintRate{} usefulness rate.
