...
%% -----------------------------------------------------------------------
\newcommand{\StrEnc}{\mathsf{StrEnc}}
\newcommand{\SymStr}{\mathsf{SymStr}}
\newcommand{\NamLaw}{\mathsf{NamLaw}}
\newcommand{\DocShad}{\mathsf{DocShad}}
\newcommand{\StreamEnc}{\mathsf{StreamEnc}}
\newcommand{\TextEnc}{\mathsf{TextEnc}}
\newcommand{\QFSLIA}{\textsf{QF\_SLIA}}
\newcommand{\QFS}{\textsf{QF\_S}}
\newcommand{\strsort}{\mathtt{String}}
\newcommand{\strlen}[1]{\mathsf{str.len}(#1)}
\newcommand{\strpre}[2]{\mathsf{str.prefixof}(#1,\,#2)}
\newcommand{\strsuf}[2]{\mathsf{str.suffixof}(#1,\,#2)}
\newcommand{\strinre}[2]{\mathsf{str.in\_re}(#1,\,#2)}
\newcommand{\strconcat}[2]{#1 \mathbin{+\!\!+} #2}
\newcommand{\strcont}[2]{\mathsf{str.contains}(#1,\,#2)}
\newcommand{\NFC}{\mathrm{NFC}}
\newcommand{\NFKC}{\mathrm{NFKC}}
\newcommand{\StrOp}{\mathsf{StrOp}}
\newcommand{\CKind}{\mathsf{CKind}}
\newcommand{\CStr}{\mathsf{CStr}}
\newcommand{\PropPair}[2]{\langle #1,\,#2 \rangle}
\newcommand{\Prop}{\mathsf{Prop}}
\newcommand{\Chunk}{\mathsf{Chunk}}
\newcommand{\strreplace}[3]{\mathsf{str.replace}(#1,\,#2,\,#3)}
\newcommand{\strindexof}[2]{\mathsf{str.indexof}(#1,\,#2)}
\newcommand{\strsubstr}[3]{\mathsf{str.substr}(#1,\,#2,\,#3)}
\newcommand{\restar}[1]{#1^{*}}
\newcommand{\replus}[1]{#1^{+}}
\newcommand{\reopt}[1]{#1^{?}}
\newcommand{\rerange}[2]{\mathsf{re.range}(#1,\,#2)}
\newcommand{\reunion}[2]{#1 \cup #2}
\newcommand{\recat}[2]{#1 \cdot #2}
...
\TextEnc{} reduces average query time by 41\% compared to na\"ive bit-vector
...
\subsection{The \StrEnc{} Data Structure}
\label{sec:strenc}
...
\[
\begin{aligned}
  \mathsf{compile}(\texttt{.})           &\;:=\; \rerange{\texttt{"\\x00"}}{\texttt{"\\U0010FFFF"}} \\
...
(\texttt{\\1}), and possessive quantifiers are not in the \QFSLIA{} regex
...
\hline
BV256           & 41.2 & 310.7 & 12 \\
QF\_S naive     & 17.4 & 89.3  &  3 \\
\TextEnc{}      & \medtime & 42.1  &  0 \\
\hline
\end{tabular}
\end{table}

\TextEnc{} achieves a median time of \medtime{} (41\% reduction over
QF\_S naive, 84\% over BV256) while eliminating all 3 failures from naive
...
\begin{itemize}
  \item 47\% to \texttt{length\_only} (P complexity, solved in $< 1$ ms),
  \item 31\% to \texttt{prefix\_suffix} or \texttt{naming\_laws} (NP),
  \item 15\% to \texttt{regex\_intersection} (PSPACE),
  \item 7\% to \texttt{mixed} (\QFSLIA{}).
\end{itemize}
The fragment-aware routing is responsible for the bulk of the speedup: 78\%
...
NFC normalisation adds at most $0.3$ ms per query on the benchmark (well under
5\% of total time).  In 6 functions involving combining characters, NFC
...
line-by-line file readers), \StreamEnc{} reduces peak Z3 heap usage by 3.1$\times$
...
Experiments show that \TextEnc{} reduces median solver time by 41\% over
