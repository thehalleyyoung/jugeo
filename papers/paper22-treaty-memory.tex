...
interface mechanisms enforce agreement only for interfaces fixed in
advance; JuGeo also needs to cope with contracts that are discovered
incrementally during verification.  We present \emph{Treaty Memory}, a
design for persisting negotiated inter-module agreements and reusing
them in later runs.  The paper develops treaty objects, negotiation
rounds, and memory-backed recall as JuGeo concepts, then evaluates the
current implementation on 10 benchmark programs.  In that benchmark,
fresh negotiation reaches 50.0\% recall, exact treaty recall reaches
55.0\%, and Jaccard-threshold recall reaches 52.5\%; exact recall also
reduces average lookup time from 10730.67\,ms to 0.10\,ms.  No
deadlocks were observed in the measured runs, so the paper's claims are
limited to persistence and replay rather than adjudication efficacy.
\end{abstract}
...
We evaluate the Treaty Memory framework on 10 multi-module
verification benchmark programs drawn from the \JuGeo{} test suite.
...
\begin{table}[H]
\centering
\caption{Main experimental results across 10 multi-module
...
\midrule
Fresh                  & \ppTwentytwoFreshRecall       & \ppTwentytwoFreshRounds & \ppTwentytwoFreshTime & \ppTwentytwoFreshDeadlock \\
Memory (exact)         & \ppTwentytwoMemExactRecall       & \ppTwentytwoMemExactRounds & \ppTwentytwoMemExactTime & \ppTwentytwoMemExactDeadlock \\
Memory ($J \geq 0.8$)  & \ppTwentytwoMemJaccardRecall & \ppTwentytwoMemJaccardRounds & \ppTwentytwoMemJaccardTime & \ppTwentytwoMemJaccardDeadlock \\
\bottomrule
\end{tabular}
\end{table}

The exact-match memory configuration achieves the highest recall
accuracy (\ppTwentytwoMemExactRecall) by reusing prior resolutions
verbatim.  The Jaccard-threshold variant (\ppTwentytwoMemJaccardRecall)
strikes a balance between recall and generalisation by recognising
structurally similar conflict patterns even when the exact friction
signature differs.  All configurations achieved a
\ppTwentytwoFreshDeadlock{} deadlock rate on this benchmark,
confirming that the negotiation protocol converges reliably.

\subsection{Adjudication Results}

Of the 48 tasks, 15 experienced at least one deadlock in fresh mode.
Of these, most were resolved by the adjudicator in a single
...
verification tasks show \subSiteSuccessRate{} site-call success rate,
a reduction in negotiation rounds, and improved deadlock
