...
\newcommand{\precRate}{94\%}               % precision rate
\newcommand{\recRate}{89\%}               % recall rate
\newcommand{\genTime}{12.3\,ms}            % average generation time
...
\midrule
Type hints only ($\SynT$) & 97\% & 72\% & 2.1 & 3.2 \\
Runtime only ($\SynR$, $N=200$) & 88\% & 81\% & 3.4 & 18.7 \\
Combined ($\SynT + \SynR$) & \precRate{} & \recRate{} & 4.0 & \genTime{} \\
...
The high precision of $\SynT$ (97\%) confirms that type annotations are
...
Adding runtime observations ($\SynR$) improves recall from 72\% to
89\%.  The improvement is concentrated in postconditions: runtime
...
$H^1 = 0$ for 91 of the 100 benchmark programs (91\%).  The 9 failures
...
Type-level synthesis alone requires only 3.2\,ms.  Pre-built registry
lookup is essentially free ($< 0.1$\,ms), making it the preferred path
...
conservative; $N = 100$ achieves 85\% recall at half the trace cost.
