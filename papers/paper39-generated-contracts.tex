...
automatic synthesizer.  In the repository benchmark of
\ppXXXIXTotalContracts{} functions, the synthesizer achieves
\precRate{} precision and \recRate{} recall with average generation time
\genTime{} per function.  This audited version limits its empirical
claim to those registry-backed measurements.
\end{abstract}
...
\caption{Pre-built contract registry: \nTotal{} expert contracts.
  These contracts generate a total of \compPropsSum{} verification
  propositions across the full benchmark suite.}
\label{tab:registry}
\begin{tabular}{llr}
\toprule
\textbf{Library} & \textbf{Category} & \textbf{Contracts} \\
\midrule
PyTorch & Linear algebra (\texttt{matmul}, \texttt{mm}, \texttt{bmm}, \ldots) & 9 \\
PyTorch & Shape manipulation (\texttt{reshape}, \texttt{transpose}, \ldots) & 8 \\
PyTorch & Neural network layers (\texttt{linear}, \texttt{conv2d}, \ldots) & 8 \\
PyTorch & Activation functions (\texttt{relu}, \texttt{softmax}, \ldots) & 7 \\
PyTorch & Loss functions (\texttt{cross\_entropy}, \texttt{mse\_loss}, \ldots) & 5 \\
\midrule
PyTorch & \emph{Subtotal} & \nPT{} \\
\midrule
NumPy & Array creation (\texttt{zeros}, \texttt{ones}, \texttt{eye}, \ldots) & 7 \\
NumPy & Linear algebra (\texttt{dot}, \texttt{matmul}, \texttt{linalg.inv}, \ldots) & 8 \\
NumPy & Aggregation (\texttt{sum}, \texttt{mean}, \texttt{max}, \ldots) & 7 \\
NumPy & Shape manipulation (\texttt{reshape}, \texttt{stack}, \ldots) & 7 \\
\midrule
NumPy & \emph{Subtotal} & \nNP{} \\
\midrule
\textbf{Total} & & \textbf{\nTotal{}} \\
...
\ppXXXIXTotalContracts{} library functions drawn from:
\begin{itemize}
  \item PyTorch: \ppXXXIXPytorchCount{} functions;
  \item NumPy: \ppXXXIXNumpyCount{} functions;
  \item Pandas: \ppXXXIXPandasCount{} functions;
  \item SciPy: \ppXXXIXScipyCount{} functions;
  \item scikit-learn: \ppXXXIXSklearnCount{} functions.
\end{itemize}
...
\begin{table}[h]
\centering
\caption{Contract synthesis evaluation on \ppXXXIXTotalContracts{} library functions.}
\label{tab:results}
\begin{tabular}{lrrrr}
\toprule
\textbf{Source} & \textbf{Precision} & \textbf{Recall} & \textbf{Avg.\ obligations} & \textbf{Avg.\ time (ms)} \\
\midrule
Type hints only ($\SynT$) & \multicolumn{4}{c}{\emph{Qualitative only in current benchmark}} \\
Runtime only ($\SynR$) & \multicolumn{4}{c}{\emph{Qualitative only in current benchmark}} \\
Combined ($\SynT + \SynR$) & \precRate{} & \recRate{} & \ppXXXIXCombinedOblig & \genTime{} \\
Pre-built registry & 100\% & 100\% & \ppXXXIXRegistryOblig & \subSiteMeanTime \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Breakdown by library for the audited benchmark.}
\label{tab:breakdown}
\begin{tabular}{llrrr}
\toprule
\textbf{Library} & \textbf{Category} & \textbf{Functions} & \textbf{Precision} & \textbf{Recall} \\
\midrule
PyTorch & Mixed & \ppXXXIXPytorchCount{} & \ppXXXIXPytorchPrec & --- \\
NumPy & Mixed & \ppXXXIXNumpyCount{} & \ppXXXIXNumpyPrec & --- \\
Pandas & Mixed & \ppXXXIXPandasCount{} & \ppXXXIXPandasPrec & --- \\
SciPy & Mixed & \ppXXXIXScipyCount{} & \ppXXXIXScipyPrec & --- \\
scikit-learn & Mixed & \ppXXXIXSklearnCount{} & \ppXXXIXSklearnPrec & --- \\
\midrule
\textbf{Overall} & & \textbf{\ppXXXIXTotalContracts} & \textbf{\precRate} & \textbf{\recRate} \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Precision/recall profile.}
The audited benchmark supports a strong precision claim
(\precRate{}) but only moderate recall (\recRate{}).  The synthesizer is
therefore conservative: emitted propositions are typically correct, but
many useful contract clauses are still missed.
...
\begin{itemize}
  ...
  \item A small audited benchmark of \ppXXXIXTotalContracts{} registry
    entries spanning NumPy, Pandas, PyTorch, SciPy, and
    scikit-learn, with \precRate{} precision and \recRate{} recall for
    the combined synthesizer.
\end{itemize}
