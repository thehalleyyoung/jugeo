...
% Timing / experiment
\newcommand{\numroute}{100}
\newcommand{\routeacc}{\subSiteSuccessRate}
\newcommand{\numfrag}{\compMorphTotal}
\newcommand{\avgsplit}{—}
\newcommand{\medroute}{\subSiteMeanTime}
...
Monolithic (Z3) & N/A & — & — & — \\
Manual tagging  & — & — & — & — \\
\JuGeo{} router & \routeacc{} & \expAccuracy & — & \medroute{} \\
\bottomrule
\end{tabular}
\end{table}
...
The router achieves \expAccuracy{} verification rate on the universal benchmark — comparable to the
expert manual baseline — while requiring no human
intervention.  The monolithic baseline is limited because some
...
The most common fragment type was \QFLIA{}, followed by
\texttt{STRINGS}, \QFUF{}, and \QFBV{}.

\paragraph{Dispatch latency.}
Median dispatch latency of \medroute{} is $8\times$ faster than
monolithic Z3 dispatch.  The speedup arises from two
...
\midrule
Restriction  & \compMorphRestriction & \compMorphRestrictionPct & \QFLIA{} / arithmetic \\
Transport    & \compMorphTransport   & \compMorphTransportPct   & Cross-theory dispatch \\
Inclusion    & \compMorphInclusion   & \compMorphInclusionPct   & Subsort embedding \\
\midrule
\textbf{Total} & \compMorphTotal & 100\% & — \\
\bottomrule
\end{tabular}
\caption{Morphism-type distribution used for evidence routing (\compMorphTotal{} total morphisms).}
\label{fig:fragments}
\end{figure}
