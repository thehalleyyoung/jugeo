...
% Timing / experiment
\providecommand{\numroute}{100}
\providecommand{\routeacc}{\ppTwentythreeJugeoRouteAcc}
\providecommand{\numfrag}{\ppTwentythreeMorphTotal}
\providecommand{\avgsplit}{---}
\providecommand{\medroute}{\ppTwentythreeJugeoLatency}
...
in a single formula.  Monolithic dispatch sends the entire \textsc{VC}
to one backend, while JuGeo's router classifies fragments and routes
them to matching solver capabilities and trust ceilings.  This paper
describes that routing model and evaluates the current implementation on
12 JuGeo benchmark programs.  The measured router reaches
\routeacc{} routing accuracy on that benchmark, preserves a 100.0\%
verification rate across the compared configurations, and reduces median
dispatch latency to \medroute{} versus 7980.93\,ms for monolithic Z3.
The empirical claim is therefore about routing overhead and fragment
selection, not about human annotation studies or external proof
...
\midrule
Monolithic (Z3) & N/A & \ppTwentythreeMonoVerif & \ppTwentythreeMonoFrags & \ppTwentythreeMonoLatency \\
Manual tagging  & \ppTwentythreeManualRouteAcc & \ppTwentythreeManualVerif & \ppTwentythreeManualFrags & \ppTwentythreeManualLatency \\
\JuGeo{} router & \ppTwentythreeJugeoRouteAcc & \ppTwentythreeJugeoVerif & \ppTwentythreeJugeoFrags & \ppTwentythreeJugeoLatency \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Routing accuracy.}
On the measured benchmark, the \JuGeo{} router achieved
\routeacc{} routing accuracy against the benchmark routing labels.
No misclassified cases were recorded in the current dataset.

\paragraph{Verification rate.}
All three measured configurations verify every benchmark program, so the
current comparison is about routing latency rather than end-to-end
success rate differences.

\paragraph{Fragment statistics.}
Across \numroute{} programs, \numfrag{} routed morphism records were
produced, with a measured mean of \ppTwentythreeJugeoFrags{} fragments
per workload.  The benchmark therefore exercises genuinely mixed
conditions rather than single-fragment obligations.

\paragraph{Dispatch latency.}
Median dispatch latency of \medroute{} is about $2\times$ faster than
the monolithic Z3 baseline (7980.93\,ms) and also below the manual
routing configuration (8135.42\,ms).  The measured advantage comes from
fragment-specific routing, not from any external human-in-the-loop step.
...
\begin{tabular}{@{}lrrl@{}}
\toprule
Morphism type & Count & Share & Analogous fragment \\
\midrule
Restriction  & \ppTwentythreeMorphRestrictionCount & \ppTwentythreeMorphRestrictionPct & \QFLIA{} / arithmetic \\
Analogue     & \ppTwentythreeMorphAnalogueCount   & \ppTwentythreeMorphAnaloguePct   & Cross-theory dispatch \\
Structural   & \ppTwentythreeMorphStructuralCount  & \ppTwentythreeMorphStructuralPct  & Subsort embedding \\
\midrule
\textbf{Total} & \ppTwentythreeMorphTotal & 100\% & --- \\
\bottomrule
\end{tabular}
\caption{Morphism-type distribution used for evidence routing (\ppTwentythreeMorphTotal{} total morphisms).}
\label{fig:fragments}
\end{figure}
...
\begin{itemize}
  \item most received bundle trust tier $\Tsolver$ — all fragments
    were Z3-discharged.
  \item some received $\Truntime$ — at least one fragment required
    a runtime witness.
  \item a few received $\Tcopilot$ — a specification-repair step
