...
\newcommand{\RelSpec}{\mathsf{RelSpec}}             % RelationSpec
\newcommand{\RelKind}{\mathsf{RelKind}}             % RelationKind enum
...
\midrule
Functional correctness & 40 & 87.5\% & 10.0\% & 2.5\% & 5.8 \\
Resource bounds        & 30 & 76.7\% & 16.7\% & 6.7\% & 7.2 \\
Security properties    & 30 & 73.3\% & 20.0\% & 6.7\% & 6.1 \\
\midrule
\textbf{Overall}       & \textbf{100} & \textbf{80.0\%} & \textbf{15.0\%} & \textbf{5.0\%} & \textbf{\medtime} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}[leftmargin=2em]
  \item The median check latency of \medtime\ is dominated by SMT
        ...
        for less than 0.4\,ms on average.
...
\begin{table}[h]
\centering
\caption{Relational refinement classification on 40 program pairs.}
\label{tab:refinement}
\begin{tabular}{lcc}
\toprule
\textbf{True class} & \textbf{Pairs} & \textbf{Correct} \\
\midrule
\texttt{FORWARD} (A refines B) & 15 & 93.3\% \\
\texttt{EQUIVALENT}            & 10 & 100.0\% \\
\texttt{INCOMPARABLE}          & 15 & 86.7\% \\
\midrule
\textbf{Overall}               & 40 & 92.5\% \\
\bottomrule
\end{tabular}
\end{table}

The 7.5\% error rate (3 misclassified pairs) all involved
...
Section~\ref{sec:incremental}, we tested 20 programs each certified
...
\begin{tabular}{ccc}
\toprule
$k$ & Median total (ms) & Marginal per $\Spec_k$ (ms) \\
\midrule
1 & 6.5  & 6.5 \\
2 & 9.1  & 2.6 \\
3 & 11.4 & 2.3 \\
...
\end{tabular}
\end{center}
The marginal cost converges to roughly 2.1\,ms per additional
specification after the first, confirming the $O(1)$ incremental
overhead claim.
...
\citeauthor{Clarke1994}~\cite{Clarke1994} decompose temporal logic
...
\citeauthor{Coquand2019}~\cite{Coquand2019}, who model type theories
