...
($+38.4\%$), followed by descent ($+27.1\%$) and trust ($+14.6\%$).
\end{abstract}
...
\begin{table}[H]
\centering
\caption{Scaling law parameters: $\Scale(\ProgSize) = a \cdot \ProgSize^{-b}$.
  $R^2$ values indicate goodness of fit.}
\label{tab:scaling}
\begin{tabular}{lcccc}
\toprule
Configuration & $a$ & $b$ & $R^2$ & Phase transition ($\ProgSize^*$) \\
\midrule
Full system   & 0.94 & 0.048 & 0.97 & 1\,024 \\
$\setminus \DescComp$ & 0.81 & 0.071 & 0.95 & 512 \\
$\setminus \TrustComp$ & 0.89 & 0.053 & 0.96 & 896 \\
$\setminus \SMTComp$  & 0.72 & 0.102 & 0.93 & 256 \\
\bottomrule
\end{tabular}
\end{table}

The steeper exponent $b$ in the ablated configurations confirms that
each component contributes to graceful scaling.
...
\begin{table}[H]
\centering
\caption{Full ablation results on \numcases\ benchmarks.
  Impact = $\AccBase - \AccAbl{C}$.  All impacts are significant at
  $p < 0.01$.}
\label{tab:ablation}
\begin{tabular}{lcccc}
\toprule
...
\midrule
Full system ($\AccBase$) & 0.847 & — & — & — \\
$\setminus \SMTComp$  & 0.463 & +0.384 & 14.7 & $< 10^{-6}$ \\
$\setminus \DescComp$ & 0.576 & +0.271 & 11.2 & $< 10^{-5}$ \\
$\setminus \TrustComp$ & 0.701 & +0.146 & 6.8  & $< 10^{-4}$ \\
$\setminus \{\SMTComp, \DescComp\}$ & 0.321 & +0.526 & 18.3 & $< 10^{-8}$ \\
$\setminus \{\SMTComp, \TrustComp\}$ & 0.389 & +0.458 & 16.1 & $< 10^{-7}$ \\
$\setminus \{\DescComp, \TrustComp\}$ & 0.511 & +0.336 & 12.9 & $< 10^{-5}$ \\
$\setminus \CompSet$ (all disabled) & 0.198 & +0.649 & 22.4 & $< 10^{-9}$ \\
\bottomrule
\end{tabular}
\end{table}
...
Key observations:
\begin{enumerate}
\item \textbf{SMT dispatch} is the single largest contributor ($+38.4\%$).
  This confirms that the majority of \JuGeo's benchmarks reduce to
  decidable SMT fragments.
\item \textbf{Descent} contributes substantially ($+27.1\%$), especially
  on the equivalence-checking sub-suite, where programs naturally
  decompose across module boundaries.
\item \textbf{Trust} has a smaller but still significant impact ($+14.6\%$),
...
$8 \times \totaltime = 15.6$\,s, comfortably within interactive
evaluation budgets.
