...
\midrule
Tiny programs ($\leq$10 lines)  & \ppXXXVITinyVerified/\ppXXXVITinyCount   & \ppXXXVITinyRate \\
Small programs (11--25 lines)    & \ppXXXVISmallVerified/\ppXXXVISmallCount & \ppXXXVISmallRate \\
Medium programs (26--60 lines)   & \ppXXXVIMediumVerified/\ppXXXVIMediumCount & \ppXXXVIMediumRate \\
Large programs ($>$60 lines)     & \ppXXXVILargeVerified/\ppXXXVILargeCount & \ppXXXVILargeRate \\
\bottomrule
\end{tabular}
\end{table}
...
$\setminus \SMTComp$ (no solver dispatch)      & \ppXXXVINoSmtSuccess & Critical: most VCs undischarged \\
$\setminus \DescComp$ (no descent)             & \ppXXXVINoDescentSuccess & High: gluing failures on multi-coordinate programs \\
$\setminus \TrustComp$ (no trust propagation)  & \ppXXXVINoTrustSuccess & Moderate: trust levels default to $\Tcopilot$ \\
\bottomrule
\end{tabular}
\end{table}
...
\item \textbf{Trust} has no measured impact on this benchmark:
  removing trust propagation still leaves 10/10 programs successful.
\item The current dataset therefore supports necessity claims for SMT
  dispatch and descent only; it does not justify stronger pairwise or
  below-random interaction claims.
\end{enumerate}

\subsection{Scope of the current benchmark}
\label{subsec:scope}

The current benchmark only reports whole-configuration success counts.
It does not include validated per-suite accuracies, methodology-loop
iterations, or timing breakdowns beyond the ablation table above, so we
do not make those stronger claims here.

% =====================================================================
\section{Conclusion}
\label{sec:conclusion}

We have presented an ablation framework for reasoning about JuGeo's
architecture.  On the audited benchmark, SMT dispatch and descent are
clearly necessary (0/10 successes when either is removed), whereas trust
propagation is not necessary for this particular dataset (10/10
successes when removed).  We therefore restrict the empirical takeaway
to those measured whole-configuration results and omit broader
statistical or methodology-loop claims.
