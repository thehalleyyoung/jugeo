...
\newcommand{\MayAlias}{\mathtt{may\text{-}alias}}  % may-alias
\newcommand{\MustAlias}{\mathtt{must\text{-}alias}} % must-alias
\newcommand{\NoAlias}{\mathtt{no\text{-}alias}}     % no-alias
...
$\approx\!31\%$ of spurious Čech obstructions seen in alias-blind runs.
\end{abstract}
...
We evaluate the alias analysis on a benchmark suite of \numcases{}
Python programs drawn from four categories:

\begin{center}
\begin{tabular}{lrr}
  \toprule
  Category & Programs & Avg.\ heap size (objects) \\
  \midrule
  Standard library utilities & 75 & 38 \\
  Data-structure algorithms & 83 & 112 \\
  Scientific computing snippets & 72 & 247 \\
  Copilot-generated code & 70 & 61 \\
  \midrule
  Total & 300 & 113 \\
  \bottomrule
\end{tabular}
...
\begin{enumerate}[noitemsep]
  \item \textbf{Blind}: no alias analysis; all pairs treated as
    \MayAlias{}.
  \item \textbf{Direct-only (L1)}: only Level-1 direct \texttt{is}
    checks.
  \item \textbf{Full ($\AD$)}: all three levels including container and
    argument aliasing.
\end{enumerate}
...
\begin{tabular}{lrrrr}
  \toprule
  Config & \NoAlias{} pairs (\%) & Must-alias (\%) & Descent obstr.\ & Time (ms) \\
  \midrule
  Blind    & 0.0   & 0.0  & 213 / 300 & 0.4 \\
  L1 only  & 61.3  & 18.7 & 168 / 300 & 3.1 \\
  Full $\AD$ & 78.4 & 18.7 & 147 / 300 & \medtime \\
  \bottomrule
\end{tabular}
\end{center}

The full $\AD$ configuration eliminates $31\%$ of the spurious Čech
obstructions present in the blind baseline ($213 \to 147$), at a
median analysis time of \medtime{} per program.  Level-2 and Level-3
detection contribute $17.1$ percentage points of additional
\NoAlias{} classification over Level-1 alone.
...
\begin{tabular}{lrr}
  \toprule
  Benchmark subset & \NoAlias{} judgments & Misses \\
  \midrule
  Standard library    & 1\,847 & 0 \\
  Data structures     & 4\,213 & 0 \\
  Scientific          & 3\,081 & 0 \\
  Copilot-generated   & 2\,104 & 0 \\
  \midrule
  Total               & 11\,245 & 0 \\
  \bottomrule
\end{tabular}
...
Zero misses across all 11\,245 no-alias judgments provide empirical
confirmation of the soundness theorem over the benchmark suite.
...
\begin{tabular}{lrr}
  ...
  \midrule
  Z3 assertions per program (mean) & 38.4 & 51.2 \\
  Z3 assertions per program (max)  & 187  & 263  \\
  Solver time per program (ms)     & 4.1  & 5.8  \\
  Unsatisfiable cores found        & 19   & 41   \\
  Lemmas proved (net new)          & 0    & 73   \\
  \bottomrule
\end{tabular}
\end{center}

The alias-aware encoding increases assertion count by $33\%$ and solver
time by $41\%$, but yields $73$ additional lemmas proved per benchmark
run — an average of $0.24$ new proofs per program.  The majority of
these proofs confirm field-invariant preservation under aliased writes.

\subsection{Obstruction Reduction Breakdown}

Of the $213 - 147 = 66$ obstructions eliminated by the full $\AD$:

\begin{center}
\begin{tabular}{lr}
  \toprule
  Elimination reason & Count \\
  \midrule
  No-alias established by Level-1 (direct) & 31 \\
  No-alias established by Level-2 (container) & 22 \\
  No-alias established by Level-3 (argument) & 13 \\
  \midrule
  Total & 66 \\
  \bottomrule
\end{tabular}
...
Container-mediated aliasing ($22/66 = 33\%$) is the second-largest
category, confirming that Python's container model creates significant
...
\begin{itemize}[noitemsep]
  ...
  \item \textbf{Site integration}: alias annotations on covering
    morphisms guide the \GLUE{} procedure and eliminate $31\%$ of
    spurious Čech obstructions.
\end{itemize}
