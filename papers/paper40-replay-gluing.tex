...
Experiments on the 10-program benchmark report relative replay
overheads instead of absolute replay times: full replay incurs
\ppXLFullPnf, while incremental, lazy, and adaptive replay record
\ppXLIncPnf, \ppXLLazyPnf, and \ppXLAdaptPnf respectively.
\end{abstract}
...
We evaluate replay gluing on the 10-program benchmark reported in
\texttt{data-paper40.tex}.  The audited measurements cover four replay
strategies: full, incremental, lazy, and adaptive.
...
\begin{tabular}{lrrrr}
\toprule
Strategy & Relative overhead & 95-th pct & Cache hit rate & Divergences \\
\midrule
$\StratFull$  & \ppXLFullPnf & \ppXLFullPnf & \ppXLFullCacheHit  & --- \\
$\StratInc$   & \ppXLIncPnf & \ppXLIncPnf & \ppXLIncCacheHit    & --- \\
$\StratLazy$  & \ppXLLazyPnf & \ppXLLazyPnf & \ppXLLazyCacheHit    & --- \\
$\StratAdapt$ & \ppXLAdaptPnf & \ppXLAdaptPnf & \ppXLAdaptCacheHit    & --- \\
\bottomrule
\end{tabular}
\end{table}

Incremental replay achieves the best balance of overhead and cache
utilisation in the current benchmark.  Full replay is slower than the
original run (\ppXLFullPnf), whereas incremental, lazy, and adaptive
replay all stay below 1$\times$ overhead.
...
\midrule
1 & 100 & \compSiteMeanBuildMs\,ms & \emph{low} \\
3 & 100 & \compProfFormalCoreSiteMeanMs\,ms & \emph{moderate} \\
5 & 100 & \compProfOrchestrateVerificationMeanMs\,ms & \emph{moderate} \\
8 & 100 & \compProfEncodeForSolverMeanMs\,ms & \emph{high} \\
\bottomrule
\end{tabular}
...
\item A replay/cache architecture whose best measured cache-hit rate is
  \ppXLIncCacheHit\ under incremental replay (\S\ref{sec:cache}).
