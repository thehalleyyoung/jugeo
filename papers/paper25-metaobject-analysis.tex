...
The repository-backed evaluation is narrower than the original draft:
on 12 benchmark programs spanning four JuGeo domains, the analysis
touches 931 classes and 178 multi-inheritance cases, reports 0 measured
metaclass conflicts, and records full ABC/descriptor discharge on the
benchmark workload.
\end{abstract}
...
We evaluated the metaobject-surface pipeline on 12 JuGeo benchmark
programs grouped into four domains: scientific computing, web
frameworks, data processing, and standard-library style utilities.  The
reported totals aggregate 931 discovered classes and 178
multi-inheritance cases across those benchmark programs.
...
\begin{table}[h]
  ...
  \begin{tabularx}{\textwidth}{lrrrrrr}
    ...
    \textbf{Domain} & \textbf{Classes} & \textbf{Diamond}
      ...
      & \textbf{Time} \\
    \midrule
    Scientific       & \ppTwentyfiveSciClasses & \ppTwentyfiveSciDiamonds & \ppTwentyfiveSciConflicts & \ppTwentyfiveSciAbc & \ppTwentyfiveSciDesc & \ppTwentyfiveSciTime \\
    Web frameworks   & \ppTwentyfiveWebClasses & \ppTwentyfiveWebDiamonds & \ppTwentyfiveWebConflicts & \ppTwentyfiveWebAbc & \ppTwentyfiveWebDesc & \ppTwentyfiveWebTime \\
    Data processing  & \ppTwentyfiveDataClasses & \ppTwentyfiveDataDiamonds & \ppTwentyfiveDataConflicts & \ppTwentyfiveDataAbc & \ppTwentyfiveDataDesc & \ppTwentyfiveDataTime \\
    Stdlib utilities & \ppTwentyfiveStdlibClasses & \ppTwentyfiveStdlibDiamonds & \ppTwentyfiveStdlibConflicts & \ppTwentyfiveStdlibAbc & \ppTwentyfiveStdlibDesc & \ppTwentyfiveStdlibTime \\
    \midrule
    \textbf{Total}   & \ppTwentyfiveTotalClasses & \ppTwentyfiveTotalDiamonds & \ppTwentyfiveTotalConflicts & \ppTwentyfiveTotalAbcOk & \ppTwentyfiveTotalDescOk & \textbf{\ppTwentyfiveMeanTime} \\
    \bottomrule
  \end{tabularx}
\end{table}
...
Across the audited benchmark, the analysis records 178
multi-inheritance cases and 0 measured metaclass conflicts.  The
benchmark therefore substantiates that the current pipeline handles the
included C3 linearisation examples without emitting conflict reports.

\paragraph{ABC obligations.}
The measured benchmark reports complete ABC discharge on all 12
programs.  This paper therefore does not claim uncovered ABC failure
modes from the current dataset.

\paragraph{Descriptor contracts.}
Descriptor checks are likewise fully discharged on the audited
benchmark, so the quantitative claim is limited to the benchmark totals
...
global guarantees without diamond-problem interference.

Experimental evaluation on 12 benchmark programs reports analysis over
931 classes, 178 multi-inheritance cases, 0 measured conflicts, and
full ABC/descriptor discharge on the included workload.  We therefore
limit the empirical claim to JuGeo's own benchmark suite and omit
unmeasured comparisons with external tools.
