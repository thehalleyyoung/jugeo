...
finds a correct fix via sheaf-guided repair with \subRepairTotal{} repair scenarios
and \subRepairObstructions{} obstructions, as validated by the Lean-verified pipeline.
\end{abstract}
...
Coherence-bug  & — & — & — & — \\
\midrule
\textbf{Overall}  & \textbf{\subRepairTotal} & \textbf{verified (Lean)} & — & — \\
\bottomrule
\end{tabular}
\end{table}
...
The repair experiments confirm that the frontier is small in
...
\midrule
\texttt{FIX\_IMPLEMENTATION}         & — \\
\texttt{STRENGTHEN\_PRECONDITION}     & — \\
\texttt{REFINE\_FUNCTION\_SPEC}       & — \\
\texttt{ADD\_INVARIANT}               & — \\
\texttt{SPLIT\_COVER}                 & — \\
\texttt{WEAKEN\_POSTCONDITION}        & — \\
\bottomrule
\end{tabular}
...
most common) reflects the benchmark composition: most seeded bugs are simple
...
iteration count on coherence-bugs when obstructions are
repaired via the sheaf-guided strategy rather than independently.
...
\begin{enumerate}[leftmargin=2em,itemsep=2pt]
  ...
  \item \textbf{Experimental validation}: \subRepairTotal{} repair scenarios with
        \subRepairObstructions{} obstructions, verified end-to-end in Lean~4.
\end{enumerate}
