...
This paper studies \emph{doctrine completion} as a JuGeo design for
detecting doctrine frontiers, synthesising candidate extensions, and
re-running verification under conservative trust accounting.  The
formal development in the paper focuses on termination and
conservativity of successful completion steps.  The repository-backed
evaluation is more modest: across 10 benchmark programs (245 site
calls), the current schema library closes 112 of 133 identified gaps
(45.7\%), with eager and lazy startup overheads below a millisecond.
The benchmark also shows clear limits: semantic gaps remain open under
the present completion strategies.
\end{abstract}
...
\midrule
\EAGER    & High          & Zero        & \ppTwentyoneEagerClosureRate \\
\LAZY     & Medium        & Low         & \ppTwentyoneLazyClosureRate \\
\DEMAND   & Zero          & Medium      & \ppTwentyoneDemandClosureRate \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
\textsc{structural}   & \ppTwentyoneStructuralVcs & \ppTwentyoneStructuralGaps & \ppTwentyoneStructuralClosed & \ppTwentyoneStructuralRate \\
\textsc{behavioral}   & \ppTwentyoneBehavioralVcs & \ppTwentyoneBehavioralGaps & \ppTwentyoneBehavioralClosed & \ppTwentyoneBehavioralRate \\
\textsc{relational}   & \ppTwentyoneRelationalVcs & \ppTwentyoneRelationalGaps & \ppTwentyoneRelationalClosed & \ppTwentyoneRelationalRate \\
\textsc{resource}     & \ppTwentyoneResourceVcs & \ppTwentyoneResourceGaps & \ppTwentyoneResourceClosed & \ppTwentyoneResourceRate \\
\textsc{semantic}     & \ppTwentyoneSemanticVcs & \ppTwentyoneSemanticGaps & \ppTwentyoneSemanticClosed & \ppTwentyoneSemanticRate \\
\midrule
\textbf{Total (site calls)} & \ppTwentyoneTotalVcs & \ppTwentyoneTotalGaps & \ppTwentyoneTotalClosed & \textbf{\ppTwentyoneTotalClosureRate} \\
\bottomrule
\end{tabular}
\end{table}

The overall site-call success rate of \subSiteSuccessRate{} is consistent with the
theoretical bound of Theorem~\ref{thm:closure-rate}: the
remaining gaps lie strictly outside the frontier
...
Median end-to-end time & \compTimeMedian \\
Minimum time           & \compTimeMin \\
Maximum time           & \compTimeMax \\
Std.\ deviation        & \compTimeStdev \\
\bottomrule
\end{tabular}
\caption{Per-program doctrine completion latency (full pipeline).}
\label{fig:latency}
\end{figure}

The sub-millisecond median latency (\subSiteMeanTime) confirms that doctrine
completion is practical for interactive verification sessions.  The
The P99 tail occurs for relational conditions requiring schema
...
\midrule
\EAGER  &  \ppTwentyoneEagerStartup & \ppTwentyoneEagerOverhead & \ppTwentyoneEagerClosureRate \\
\LAZY   &   \ppTwentyoneLazyStartup & \ppTwentyoneLazyOverhead & \ppTwentyoneLazyClosureRate \\
\DEMAND &    \ppTwentyoneDemandStartup & \ppTwentyoneDemandOverhead & \ppTwentyoneDemandClosureRate \\
\bottomrule
\end{tabular}
...
\DEMAND\ sacrifices some closure rate in exchange
...
baseline verification rate established by the universal benchmark (\expAccuracy).  With doctrine completion
(\LAZY\ strategy), all \expTotalPrograms{} benchmark programs verify successfully.
The remaining gap between the site-call success rate (\subSiteSuccessRate) and full closure is attributable to
...
\begin{itemize}[leftmargin=2em]
  ...
  \item Experimental validation showing \subSiteSuccessRate{} site-call success and
        sub-millisecond median latency (see \S\ref{sec:experiments}) on \numcases\ benchmark programs.
\end{itemize}
