...
Experiments on the repository benchmark cover \ppXXXVTotalPrograms{}
programs across five partiality categories.  The audited comparison is
between a total-by-default baseline and the repository's PMR analysis:
the mean baseline false-termination rate is \ppXXXVMeanBaseFtr, while the
mean PMR false-termination rate is \ppXXXVMeanPmrFtr.
\end{abstract}
% ─────────────────────────────────────────────────────────────────────
...
These four partiality sources appear frequently in the repository
benchmark as well: exception-style partiality appears in
\ppXXXVExceptPct{} of the modelled functions and implicit partiality in
\ppXXXVImplicitPct{}.  Treating them as total in a verification context
leads to \emph{false termination claims}.
...
\midrule
Non-termination (possible)   & \ppXXXVNontermFuncs & \ppXXXVNontermPct \\
Uncaught exceptions          & \ppXXXVExceptFuncs & \ppXXXVExceptPct \\
Implicit \texttt{None}       & \ppXXXVImplicitFuncs & \ppXXXVImplicitPct \\
Precondition violation       & \ppXXXVPrecondFuncs & \ppXXXVPrecondPct \\
Undefined binding            & \ppXXXVUndefFuncs & \ppXXXVUndefPct \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{lstlisting}[language=Python, caption={Using the \JuGeo{} repair semantics API to reconstruct a partial section over a site.}]
from jugeo.geometry import SiteBuilder

code = open("partial_functions.py").read()
site = SiteBuilder(code).build()

print(f"Coordinates:    {site.coordinate_count()}")
print(f"Covering families: {len(site.covering_families())}")

# Run the partiality reconstruction (PMR completion)
repair = site.repair_semantics()
print(f"Observed coords:    {repair['observed_coordinate_count']}")
print(f"Reconstructed:      {repair['reconstructed_coordinate_count']}")
print(f"Unresolved partial: {repair['unresolved_count']}")
print(f"Obstruction classes:{repair['obstruction_class_count']}")
print(f"False term rate:    {repair['false_termination_rate']:.4f}")

for obs in repair.get("unresolved_obstructions", []):
    print(f"  coord={obs['coordinate']}  cause={obs['cause']}"
          f"  trust={obs['trust']}")
\end{lstlisting}
...
We evaluate \PMR{} on the repository benchmark produced by
\texttt{experiments/exp35\_partiality\_models.py}: \ppXXXVTotalPrograms{}
small programs, two in each of the five partiality categories described in
\cref{sec:partiality}.  For each program we collect the JuGeo descent verdict,
the number of reported obstructions, and the number of local observations
available to the reconstruction.  The audited comparison is between:
\begin{itemize}
  \item \textbf{Total-by-default}: a baseline that treats each function as
        total unless evidence of failure is already present.
  \item \textbf{\PMR{}}: the repository's conservative partiality-aware
        reconstruction, which lowers the false-termination rate by assigning
        $\bot$ to unsupported cases.
\end{itemize}

\begin{lstlisting}[language=Python, caption={Running the \JuGeo{} partiality analysis via the CLI and comparing reconstruction quality.}]
import subprocess, json

result = subprocess.run(
    ["python3", "-m", "jugeo", "--format", "json",
     "partiality", "partial_functions.py"],
    capture_output=True, text=True
)
data = json.loads(result.stdout)

print(f"Functions analysed:    {data['function_count']}")
print(f"False termination rate:{data['false_termination_rate']:.4f}")
print(f"Precision:             {data['precision']:.4f}")
print(f"Recall:                {data['recall']:.4f}")
print(f"Median analysis time:  {data['median_time_ms']:.2f} ms")

# Per-category partiality breakdown
for cat in data.get("category_breakdown", []):
    print(f"  {cat['category']:20s}  FTR={cat['false_termination_rate']:.2f}"
          f"  recall={cat['recall']:.2f}")
\end{lstlisting}
...
\begin{table}[ht]
\centering
\caption{Aggregate false-termination metrics on the
  \ppXXXVTotalPrograms{}-program repository benchmark.}
\label{tab:results}
\begin{tabular}{lr}
\toprule
Metric & Value \\
\midrule
Programs analysed & \ppXXXVTotalPrograms \\
Mean baseline FTR & \ppXXXVMeanBaseFtr \\
Mean PMR FTR & \ppXXXVMeanPmrFtr \\
Mean observations/function & \ppXXXVMeanObsPerFunc \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}[label=(\roman*)]
  \item The audited benchmark is small but structured: each partiality
        category contributes two programs, so the per-category rows are
        directly comparable.
  \item The mean false-termination rate drops from \ppXXXVMeanBaseFtr{} for
        the total-by-default baseline to \ppXXXVMeanPmrFtr{} for \PMR{},
        showing that the partiality-aware reconstruction is materially more
        conservative on the repository benchmark.
  \item Every category in Table~\ref{tab:by-cat} has the same mean observation
        count, so the category differences come from the partiality model
        rather than from unequal sampling depth.
\end{enumerate}
...
\begin{table}[ht]
\centering
\caption{False-termination rate by partiality category on the repository
  benchmark.}
\label{tab:by-cat}
\begin{tabular}{lrrrr}
\toprule
...
\midrule
Non-termination         & \ppXXXVNontermN & \ppXXXVNontermBaseFtr & \ppXXXVNontermPmrFtr & \ppXXXVNontermObsPerFunc \\
Uncaught exceptions     & \ppXXXVExceptN & \ppXXXVExceptBaseFtr & \ppXXXVExceptPmrFtr & \ppXXXVExceptObsPerFunc \\
Implicit \texttt{None}  & \ppXXXVImplicitN & \ppXXXVImplicitBaseFtr & \ppXXXVImplicitPmrFtr & \ppXXXVImplicitObsPerFunc \\
Precondition violation  & \ppXXXVPrecondN & \ppXXXVPrecondBaseFtr & \ppXXXVPrecondPmrFtr & \ppXXXVPrecondObsPerFunc \\
Undefined binding       & \ppXXXVUndefN & \ppXXXVUndefBaseFtr & \ppXXXVUndefPmrFtr & \ppXXXVUndefObsPerFunc \\
\bottomrule
\end{tabular}
\end{table}

The category table shows that the baseline behaves worst on implicit
\texttt{None} and precondition-violation cases, while PMR reduces the
false-termination rate to a uniform \ppXXXVMeanPmrFtr{} across the audited
programs.  The observation count remains constant at
\ppXXXVMeanObsPerFunc{} per program, so the primary effect is the
reconstruction policy rather than additional evidence.

\subsection{Scalability}

The current repository benchmark is too small for a meaningful scalability
study.  What it does show is that, with
\ppXXXVMeanObsPerFunc{} observations per program, the PMR pass is lightweight
enough to be embedded inside the broader JuGeo verification workflow without
