...
eight-tuple judgments equipped with a rich deduction-rule calculus.
Selecting the right deduction rule at each proof step remains a
bottleneck for both automated solvers and human users.  We present
\emph{Copilot-Assisted Deduction}, an architecture that integrates
large language model~(\LLM) capabilities into \JG{}'s deduction
engine.  Three components are introduced: $\CDA$ orchestrates
proof-level assistance including rule suggestion, transition
explanation, and proof completion; $\CRS$ ranks candidate rule
schemas by relevance and incorporates user feedback; and $\CC$
exposes each capability through a discoverable manifest.  We prove
that the architecture preserves deduction soundness, that
suggestion relevance converges under feedback, and that the
capability manifest is complete.  Experiments on
\ppLXXXVIIruleSuggestionsTotal{} rule suggestions across
\ppLXXXVIIproofCompletionsAttempted{} proof attempts show an
acceptance rate of \ppLXXXVIIacceptanceRate{}, a proof completion
rate of \ppLXXXVIIproofCompletionRate{}, and a median suggestion
latency of \ppLXXXVIImedianSuggestionLatencyMs{}.
\end{abstract}
...
\begin{enumerate}
  ...
  \item We prove four formal properties: \emph{soundness
    preservation} (Theorem~\ref{thm:soundness}), \emph{suggestion
    relevance} (Theorem~\ref{thm:relevance}), \emph{cache
    consistency} (Theorem~\ref{thm:cache}), and \emph{feedback
    convergence} (Theorem~\ref{thm:feedback}).  All proofs are
    mechanised in Lean~4.

  \item We evaluate the system on
    \ppLXXXVIIruleSuggestionsTotal{} suggestions across
    \ppLXXXVIIproofCompletionsAttempted{} proof attempts, measuring
    an acceptance rate of \ppLXXXVIIacceptanceRate{} and a
    top-three accuracy of \ppLXXXVIItopThreeAccuracy{}.
\end{enumerate}
...
\]
In the implementation, $\RS$ is a dataclass in
\texttt{jugeo.encodings.deduction\_rules.inference\_rules}:

\begin{lstlisting}[style=jugeo-python,caption={The $\RS$ dataclass.},
...
# A rule schema for weakening:
weakening = RuleSchema(
    name="weakening",
    premises=PremiseSet(["?Gamma |- ?A"]),
    conclusion=ConclusionForm("?Gamma, ?B |- ?A"),
    side_conditions=[],
)
\end{lstlisting}
