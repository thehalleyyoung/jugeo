...
% ─────────────────────────────────────────────────────────────────────
\begin{abstract}
Large language models (LLMs) can supply useful \emph{evidence} for
program verification---suggested invariants, contract summaries,
complexity bounds---but their outputs carry no intrinsic soundness
guarantee.  We present the \emph{Copilot evidence channel}, a
first-class integration of LLM-generated evidence into the
Judgment Geometry framework with a hard trust ceiling that prevents
LLM suggestions from silently inflating the trust level of a judgment.
The design centres on four components:
(i)~the $\EvCh$ enum, which classifies evidence sources into
    $\mathsf{SOLVER}$, $\mathsf{RUNTIME}$, $\mathsf{ORACLE}$,
    $\mathsf{COPILOT}$, $\mathsf{FORMAL\_PROOF}$, $\mathsf{HUMAN}$,
    and $\mathsf{COMPOSED}$;
(ii)~the $\CopCh$ class, which wraps \queryLLM{} calls behind
    rate limiting and a configurable timeout;
(iii)~the $\CopGate$ trust gateway, which enforces the ceiling
    $\TrustCeil$ and blocks every promotion attempt that lacks
    independent corroboration; and
(iv)~the $\CopAdapt$ adapter, which routes queries to
    heterogeneous model back-ends while tracking per-model token
    budgets.
We prove three formal properties---\emph{trust-ceiling enforcement},
\emph{no-silent-promotion}, and \emph{audit completeness}---and
verify them in \LEAN.
Experiments on \ppLXXXVIIItotalQueries{} queries across
\ppLXXXVIIIadapterCount{} model adapters confirm that the ceiling
holds in \ppLXXXVIIIceilingHeldPct{} of routed evidence,
rate limiting rejects \ppLXXXVIIIrateLimitHitPct{} of burst traffic,
and the audit log is \ppLXXXVIIIauditComplete{} complete.
The full pipeline is implemented in the \JuGeo{} \Python{} library
...
\item \textbf{Formal guarantees.}
  We state and prove three theorems---trust-ceiling enforcement,
  no-silent-promotion, and audit completeness---and formalize
  them in \LEAN{} (\S\ref{sec:formal}).

\item \textbf{Empirical evaluation.}
  We exercise the pipeline on \ppLXXXVIIItotalQueries{} queries,
  confirming that the ceiling holds universally, rate limiting
  behaves as specified, and corroboration lifts
  \ppLXXXVIIIcorrobRate{} of Copilot evidence to higher tiers
