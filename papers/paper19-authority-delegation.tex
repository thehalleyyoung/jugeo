...
roughly $30\%$ of generated chains contain at least one escalation
attempt.
...
\midrule
Ceiling breach    & 178 & 178 \\
Jurisdiction viol & 89  & 89  \\
Bad issuer        & 34  & 34  \\
Incompat type     & 18  & 18  \\
\midrule
\textbf{Total}    & 319 & 319 \\
...
Table~\ref{tab:detection} shows 100\% detection with zero false
negatives and zero false positives.  The two-dimensional ceiling +
jurisdiction check is sufficient to catch all injected violations.
...
M2 chip is $0.8\,\mu\text{s}$ mean and $1.4\,\mu\text{s}$ at the
95th percentile.
...
\midrule
\textsc{Issue}    & 0.6 & 1.1 \\
\textsc{Delegate} & 0.8 & 1.4 \\
\textsc{Revoke}   & 0.4 & 0.7 \\
\textsc{Lookup}   & 0.2 & 0.3 \\
\bottomrule
\end{tabular}
\end{table}
...
$0.02\%$ of total judgment processing time.

\subsection{Audit Log Replay}

We replayed the full audit log of the benchmark run (3\,412 events)
and confirmed that every \textsc{Grant} event satisfies the ceiling
invariant.  The replay takes $9.1\,\text{ms}$ for the full log,
or roughly $2.7\,\mu\text{s}$ per event.
...
confirm 100\% violation detection and negligible runtime overhead.
