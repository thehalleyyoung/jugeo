...
\begin{table}[h]
\centering
\caption{Aggregate violation detection on the escalation stress test.}
\label{tab:detection}
\begin{tabular}{lrr}
\toprule
Measured scope & Injected & Detected \\
\midrule
  All injected delegation violations & \ppNineteenViolationsInjected{} & \ppNineteenViolationsDetected{} \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:detection} summarizes the aggregate number of delegation
violations flagged during stress testing.  The qualitative conclusion remains
the same: the kernel never silently permits an escalation beyond the declared
ceiling, consistent with Theorem~\ref{thm:no-escalation}.
...
\begin{table}[h]
\centering
\caption{Aggregate enforcement overhead.}
\label{tab:overhead}
\begin{tabular}{lrr}
\toprule
Operation & Mean ($\mu$s) & p95 ($\mu$s) \\
\midrule
  Delegation enforcement & \ppNineteenMeanEnforceTime{} & \ppNineteenPninetyFiveEnforceTime{} \\
\bottomrule
\end{tabular}
\end{table}
