...
\providecommand{\restriction}[2]{\res_{#1}^{#2}} % restriction map from #1 to #2
...
The current repository-backed results should be read as an exploratory
evaluation of JuGeo's async-effect decomposition rather than as a
complete proof of race detection coverage.
\end{abstract}
...
\begin{table}[H]
  ...
  \begin{tabular}{lrrrr}
    ...
    \midrule
    Pure async           & \ppTwentyfourPureAcc & \ppTwentyfourPureClean   & \ppTwentyfourPureDirty  & \ppTwentyfourPureFalsePos \\
    Shared-state (benign) & \ppTwentyfourSharedBenignAcc & \ppTwentyfourSharedBenignClean  & \ppTwentyfourSharedBenignDirty    & \ppTwentyfourSharedBenignFalsePos \\
    Shared-state (races) & \ppTwentyfourSharedRacesAcc & \ppTwentyfourSharedRacesClean   & \ppTwentyfourSharedRacesDirty   & \ppTwentyfourSharedRacesFalsePos \\
    Mixed IO             & \ppTwentyfourMixedIoAcc & \ppTwentyfourMixedIoClean   & \ppTwentyfourMixedIoDirty    & \ppTwentyfourMixedIoFalsePos \\
    \midrule
    Overall (universal benchmark) & \ppTwentyfourOverallAccuracy & \ppTwentyfourOverallClean & \ppTwentyfourOverallDirty & \ppTwentyfourOverallFalsePos \\
    \bottomrule
  \end{tabular}
\end{table}

\paragraph{Effect classification.}
The current benchmark contains \ppTwentyfourTotalPrograms{} programs and
reports an overall effect-classification accuracy of
\expAccuracy{}.  The prototype cleanly handles the pure and benign
shared-state categories (100.0\% each), but it does not yet classify the
seeded shared-race or mixed-IO examples correctly (0.0\% in both
categories).

\paragraph{Boundary detection.}
The measured run records no dirty boundaries and no false positives.
That makes the current implementation conservative but incomplete on the
seeded race benchmark: it is not yet surfacing the intended dirty
boundaries for the race-heavy categories.

\paragraph{Performance.}
Mean analysis time is \expTimeMean{} across the benchmark.  Given the
50.0\% classification result and zero dirty-boundary detections, this
cost should be interpreted as prototype overhead rather than validated
...
The resulting modular proof principle reduces global verification to:
\begin{enumerate}
...
The benchmarked implementation remains incomplete for the harder async
cases: it reaches 50.0\% overall classification accuracy and does not yet
mark the seeded shared-race or mixed-IO examples as dirty.  The audited
claim for this repository is therefore limited to the decomposition
machinery and the measured prototype behaviour above.
...
obstruction.  The universal benchmark of \expTotalPrograms{} programs achieves \expAccuracy{} verification
accuracy with \expObstructionTotal{} obstructions.
