...
\ppFortyFourTotalPrograms{} benchmark programs; a fully machine-verified proof is provided
...
\begin{enumerate}
  ...
  \item Experimental validation on \ppFortyFourTotalPrograms{} benchmark programs
...
\begin{lstlisting}[style=jugeo-python, caption={JudgmentProduct dataclass
...
\begin{lstlisting}[style=jugeo-python, caption={JudgmentAlgebra.restrict
...
\begin{lstlisting}[style=jugeo-python, caption={JudgmentAlgebra.transport.},
  label=lst:transport]
@staticmethod
...
\begin{lstlisting}[style=jugeo-python, caption={JudgmentAlgebra.compose.},
  label=lst:compose]
@staticmethod
...
\begin{lstlisting}[style=jugeo-python, caption={JudgmentAlgebra.merge.},
  label=lst:merge]
@staticmethod
...
\begin{lstlisting}[style=jugeo-python, caption={JudgmentAlgebra.compare\_trust.},
  label=lst:compare]
@staticmethod
...
\begin{lstlisting}[style=jugeo-python, caption={EvidenceBundle.merge
  (from \texttt{judgment\_terms.py}).},
...
We evaluate the judgment product implementation on \ppFortyFourTotalPrograms{} benchmark
programs.  For each program we construct three product types:
conjunction, disjunction, and conditional
(\ppFortyFourTotalProducts{} products total).  For each program we construct a product of
...
\begin{table}[h]
\centering
\caption{Judgment product assembly on \ppFortyFourTotalPrograms{} benchmarks.
  ``Count'' = number of products constructed.
  ``Mean Trust'' = mean trust level (normalised to $[0,1]$).
  ``Discharged'' = fraction with trust $\geq$ SOLVER\_DISCHARGED.}
\label{tab:results}
\begin{tabular}{lrrr}
\toprule
\textbf{Product type} & \textbf{Count} &
  \textbf{Mean Trust} & \textbf{Discharged (\%)} \\
\midrule
Conjunction  & \ppFortyFourConjCount{}  & \ppFortyFourConjMeanTrust{}  & \ppFortyFourConjDischargedRate{} \\
Disjunction  & \ppFortyFourDisjCount{}  & \ppFortyFourDisjMeanTrust{}  & \ppFortyFourDisjDischargedRate{} \\
Conditional  & \ppFortyFourCondCount{}  & \ppFortyFourCondMeanTrust{}  & \ppFortyFourCondDischargedRate{} \\
\midrule
\textbf{All} & \ppFortyFourTotalProducts{} & \ppFortyFourMeanTrust{} & \ppFortyFourDischargedRate{} \\
\bottomrule
\end{tabular}
\end{table}
...
\ppFortyFourDischargedTotal{} trust obligations are discharged
(\ppFortyFourDischargedRate{}), confirming the correctness of
...
Product assembly takes a median of \ppFortyFourMedianProductTime{} per product, with a
...
(mean \ppFortyFourMeanProductTime{}).
...
the theory on \ppFortyFourTotalPrograms{} benchmark programs with 100\% pass rates on
