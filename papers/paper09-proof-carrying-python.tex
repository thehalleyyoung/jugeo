...
mod = Coordinate(("sort_merge",), kind=CoordinateKind.MODULE)
fn_merge = Coordinate(
    ("sort_merge", "merge"), kind=CoordinateKind.FUNCTION,
)
fn_merge_sort = Coordinate(
    ("sort_merge", "merge_sort"), kind=CoordinateKind.FUNCTION,
)

site = (
    SiteBuilder("sort_merge")
    .add_coordinate(mod)
    .add_coordinate(fn_merge)
    .add_coordinate(fn_merge_sort)
    ...
    .add_morphism(Morphism(
        source=fn_merge_sort,
        target=mod,
        kind=MorphismKind.INCLUSION,
    ))
    .add_morphism(Morphism(
        source=fn_merge,
        target=fn_merge_sort,
...
cover = Cover(
    mod,
    patches=(fn_merge, fn_merge_sort),
    overlaps=((fn_merge.key, fn_merge_sort.key),),
)
sections = {
    fn_merge.key: {"sorted_output": True},
    fn_merge_sort.key: {"sorted_output": True},
}
...
assert result.unwrap_section().certificate
\end{lstlisting}
...
\begin{lstlisting}[style=jugeo-python,caption={Inspecting the successful global section.}]
global_section = result.unwrap_section()

print(global_section.coordinate)
print(global_section.certificate[:16] + "...")
print(int(global_section.constituent_count))
print(global_section.trust_floor)
\end{lstlisting}
...
global certificate commits to the glued section, and restriction
consistency holds.
