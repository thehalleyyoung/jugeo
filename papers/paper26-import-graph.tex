...
We evaluate \DA{} on three Python codebases of increasing size:

\begin{enumerate}[label=(\roman*)]
  \item \textbf{JuGeo core} (this project): 42 modules, 87 edges.
  \item \textbf{NumPy 1.26}: 312 modules, 1,041 edges.
  \item \textbf{Django 4.2}: 589 modules, 2,317 edges.
\end{enumerate}

Experiments run on a laptop with Apple M2 processor, 16\,GB RAM,
macOS~14.2.
...
\begin{table}[H]
\centering
\caption{Import graph construction and analysis times.}
\label{tab:times}
\begin{tabular}{lrrrr}
\toprule
\textbf{Codebase} & $|V|$ & $|E|$ &
  \textbf{Build (ms)} & \textbf{Analysis (ms)} \\
\midrule
JuGeo core & 42  & 87    & — & — \\
NumPy      & 312 & 1041  & — & — \\
Django     & 589 & 2317  & — & — \\
...
Construction time scales near-linearly in $|V| + |E|$, consistent with
the $O(|V|+|E|)$ complexity of the AST traversal and Tarjan's
algorithm.
...
\begin{table}[H]
\centering
\caption{Cyclic import detection results.}
\label{tab:cycles}
\begin{tabular}{lrrrr}
\toprule
\textbf{Codebase} & \textbf{SCCs} & \textbf{Cyclic} &
  \textbf{Max size} & \textbf{Obstructions} \\
\midrule
JuGeo core & 42  & 0  & 1 & 0 \\
NumPy      & 308 & 4  & 3 & 4 \\
Django     & 551 & 38 & 7 & 38 \\
...
JuGeo core is acyclic by design.  Django's 38 cyclic SCCs correspond
to known circular imports in its ORM and URL routing subsystems.
\DA{} detects all of them and emits obstruction records with
$\trust = \Tsolver$ after Z3 confirms unsatisfiability of the rank
constraint system.
...
Topological verification processes each module exactly once; total
verification time decreases significantly under topological ordering.
...
The \emph{Compositionality Theorem}
(Theorem~\ref{thm:compositionality}) formalises the guarantee:
bottom-up verification in topological order is sound and complete.
...
The theorem is fully mechanized in \LEAN{} without \texttt{sorry}.

Cyclic imports—which Python permits but proof tools must handle
carefully—are resolved by SCC condensation.  The cycle is recorded
as a site obstruction, and the entire SCC is verified as an atomic
bundle judgment.  The obstruction carries an SMT-discharged
certificate at trust level $\Tsolver$.
