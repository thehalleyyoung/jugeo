...
% ════════════════════════════════════════════════════════════════════
\begin{abstract}
Judgment Geometry organises program reasoning as local sections of a
presheaf over a semantic site.  Constructing these local sections
requires choosing candidates, negotiating interface constraints, and
verifying sheaf descent---tasks that are labour-intensive yet highly
structured.  We introduce \CCP{}, a protocol that allows a
large-language-model (LLM) copilot to participate in local-section
generation while respecting the trust architecture of the JuGeo
framework.  The copilot proposes candidates via three strategies
($\mathsf{solver}$, $\mathsf{analogy}$, $\mathsf{enumeration}$),
mediates interface negotiations between overlapping construction loops,
and adaptively tunes its strategy in response to acceptance feedback.
We prove four formal properties---trust-ceiling preservation,
negotiation convergence, adaptation monotonicity, and descent
compatibility---and verify them in Lean~4.  Experiments on
\ppLXXXVInumSessions{} sessions comprising \ppLXXXVInumGoals{} goals
show a \ppLXXXVIacceptRate{} acceptance rate with
\ppLXXXVInegSuccessRate{} negotiation success, while every accepted
proposal satisfies sheaf descent.
\end{abstract}
...
We formalise these constraints as four theorems
(Section~\ref{sec:formal}), prove them in Lean~4, and validate them
experimentally on \ppLXXXVInumSessions{} construction sessions
encompassing \ppLXXXVInumGoals{} goals.  The copilot achieves
an acceptance rate of \ppLXXXVIacceptRate{} with a median proposal
time of \ppLXXXVImedianProposalTime{}, and every accepted proposal
passes sheaf-descent verification.
...
\begin{itemize}
  ...
  \item Four theorems---trust ceiling, convergence, monotonicity,
        descent---proved in Lean~4 (Section~\ref{sec:formal}).
  \item An empirical evaluation on \ppLXXXVInumGoals{} goals across
        \ppLXXXVInumSessions{} sessions (Section~\ref{sec:expt}).
\end{itemize}
