...
We implement the algebra in \Python{} and evaluate on \compTotalPrograms{}
benchmark programs, verifying all \compTotalVerified{} with
\compOverallAccuracy{} accuracy and discharging all \compTrustTotal{}
proof obligations (\compTrustSolverdischargedPct{} solver-discharged).
The algebra preserves all eight fields through every operation---a
$2.0\times$ information advantage over the best competing system
(\Fstar{}, which retains four of eight components).  Full verification
completes in \compTimeMean{} mean (\compTimeMedian{} median), with
individual judgment operations at \compMethodsMeanMs{}\,ms mean,
confirming negligible per-operation overhead.
\end{abstract}
...
\begin{enumerate}
  ...
  \item An implementation in \Python{} with evaluation on \compTotalPrograms{} programs,
        discharging all \compTrustTotal{} proof obligations
...
\paragraph{Minimal user-facing construction.}
The fluent API exposes the same structure without requiring callers to
manually assemble every nested field:

\begin{lstlisting}[style=jugeo-python,caption={Building a judgment with the fluent API.}]
from jugeo.geometry.site import Coordinate, CoordinateKind
from jugeo.judgments.judgment_terms import (
    JudgmentBuilder, Proposition, PropositionKind, TrustLevel,
)

coord = Coordinate(("demo", "inc"), kind=CoordinateKind.FUNCTION)
judgment = (
    JudgmentBuilder()
    .at(coord)
...
print(judgment.coordinate.name, judgment.proposition.formula)
\end{lstlisting}
...
We evaluate the judgment 8-tuple on \compTotalPrograms{} \Python{} programs
spanning multiple domains (algorithms, data structures, control flow,
string processing, and more).  The evaluation
discharges all \compTrustTotal{} proof obligations
(\compTrustSolverdischargedPct{} solver-discharged) and measures
proposition counts per coordinate type.
...
\begin{table}[t]
\centering
\caption{Verification results on \compTotalPrograms{} \Python{} programs.}\label{tab:results}
\begin{tabular}{@{}lrrl@{}}
\toprule
\textbf{Task family} & \textbf{Programs} & \textbf{Verified}
  & \textbf{Accuracy} \\
\midrule
Specification satisfaction & \compTotalPrograms{} & \compTotalVerified{} & \compOverallAccuracy{} \\
Behavioral equivalence     & \compEquivPairs{} pairs & \compEquivBothVerified{} & 100\% \\
Bug detection              & \compBuggyCount{} & \compBuggyFullPass{} & 100\% \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[t]
\centering
\caption{Verification latency statistics.}\label{tab:latency}
\begin{tabular}{@{}lr@{}}
\toprule
\textbf{Metric} & \textbf{Time} \\
\midrule
Full verification (mean)     & \compTimeMean{} \\
Full verification (median)   & \compTimeMedian{} \\
Full verification (min)      & \compTimeMin{} \\
Full verification (max)      & \compTimeMax{} \\
Equivalence checking (mean)  & \compEquivMeanTime{} \\
Site construction (mean)     & \compSiteMeanBuildMs{}\,ms \\
Judgment operations (mean)   & \compMethodsMeanMs{}\,ms \\
\bottomrule
\end{tabular}
\end{table}

All \compTotalPrograms{} programs are verified with \compOverallAccuracy{}
accuracy.  Full verification completes in \compTimeMean{} mean
(\compTimeMedian{} median), with a total wall-clock time of
\compTimeTotal{} for the full suite.  Individual judgment operations
run in \compMethodsMeanMs{}\,ms mean, confirming sub-millisecond
per-operation overhead.

\subsection{Subsystem method profiling}\label{ssec:op-perf}

We profile \compMethodsTested{} subsystem methods to characterize
per-operation cost.  Table~\ref{tab:op-perf} reports wall-clock time
for the most expensive methods, measured across all \compTotalPrograms{}
programs.

\begin{table}[t]
\centering
\caption{Subsystem method profiling (\compMethodsTested{} methods,
  \compMethodsSuccessful{} successful).  All \compPropsOkSum{}/\compPropsSum{}
  properties verified across \compTotalPrograms{} programs.}
\label{tab:op-perf}
\begin{tabular}{@{}lr@{}}
\toprule
\textbf{Method} & \textbf{Mean time (ms)} \\
\midrule
\texttt{encode\_for\_solver}              & \compProfEncodeForSolverMeanMs{} \\
\texttt{formal\_core\_site}               & \compProfFormalCoreSiteMeanMs{} \\
\texttt{specification\_satisfaction}       & \compProfSpecificationSatisfactionMeanMs{} \\
\texttt{orchestrate\_verification}         & \compProfOrchestrateVerificationMeanMs{} \\
All other methods (mean)                   & \compMethodsMeanMs{} \\
\bottomrule
\end{tabular}
\end{table}
...
Second, the most expensive individual method is SMT encoding
(\compProfEncodeForSolverMeanMs{}\,ms), which translates judgment
obligations into solver-ready formulas.  Site construction
(\compProfFormalCoreSiteMeanMs{}\,ms) and specification satisfaction
checking (\compProfSpecificationSatisfactionMeanMs{}\,ms) follow.
The remaining methods complete in under 0.01\,ms, confirming that
the algebraic operations themselves impose negligible overhead.

Third, all \compPropsOkSum{} of \compPropsSum{} properties are verified
across the benchmark suite, with \compObstructionTotal{} obstructions
detected---consistent with the fact that all benchmark programs are
well-formed.  The total of \compMorphTotal{} morphisms decompose into
\compMorphRestriction{} restriction (\compMorphRestrictionPct{}),
\compMorphTransport{} transport (\compMorphTransportPct{}), and
\compMorphInclusion{} inclusion (\compMorphInclusionPct{}) morphisms.
...
judgment representations.  Table~\ref{tab:comparison} (Section~\ref{ssec:deep-api})
provides a detailed, per-dimension comparison showing that \LEAN{} retains
3 components, \Fstar{} retains 4, \Dafny{} retains 3 (with partial
counterexample support), and \JuGeo{} retains all~8.
...
We verify the trust algebra axioms across all \compTotalPrograms{} benchmark
programs.  The axioms are structural properties of the ordered algebra
$(\mathcal{T}, \tleq, \tjoin)$ that hold by construction; we confirm
empirically that all \compTrustTotal{} proof obligations are
solver-discharged (\compTrustSolverdischargedPct{}).

\begin{table}[t]
\centering
\caption{Trust algebra axiom verification.  All \compTrustTotal{}
  obligations discharged across \compTotalVerified{} verified programs.}
\label{tab:axioms}
\begin{tabular}{@{}lcp{7cm}@{}}
\toprule
\textbf{Axiom} & \textbf{Status} & \textbf{Description} \\
\midrule
Reflexivity          & \checkmark & $\forall t.\; t \tleq t$ \\
Transitivity         & \checkmark & $t_1 \tleq t_2 \wedge t_2 \tleq t_3 \Rightarrow t_1 \tleq t_3$ \\
Antisymmetry         & \checkmark & $t_1 \tleq t_2 \wedge t_2 \tleq t_1 \Rightarrow t_1 = t_2$ \\
Meet exists          & \checkmark & $\forall t_1, t_2.\; \exists\, t_1 \tjoin t_2$ \\
Oracle ceiling       & \checkmark & $\Tcopilot \tleq \tau_{\max}$ for oracle-provided evidence \\
Monotonicity         & \checkmark & Algebraic operations preserve trust ordering \\
Contradicted absorbs & \checkmark & $\Tcontra \tjoin t = \Tcontra$ for all $t$ \\
\midrule
\textbf{Verification} & \multicolumn{2}{l}{\compTrustSolverdischarged{}/\compTrustTotal{} obligations discharged (\compTrustSolverdischargedPct{})} \\
\bottomrule
\end{tabular}
\end{table}

The universal pass confirms that the trust algebra axioms hold not
merely in theory but across all \compTotalPrograms{} programs.  The
...
coordinate kind, aggregated over the \compTotalPrograms{} benchmark programs.

\begin{table}[t]
\centering
\caption{Average proposition counts per coordinate type across \compTotalPrograms{}
  programs.  All coordinate types carry a uniform \ppTWOoverallCoordMeanProps{}
  propositions per coordinate (structural plus behavioral).}
\label{tab:prop-counts}
\begin{tabular}{@{}lrp{5.5cm}@{}}
\toprule
\textbf{Coordinate kind} & \textbf{Avg.\ props}
  & \textbf{Dominant proposition kind} \\
\midrule
\texttt{MODULE}    & \ppTWOmoduleCoordMeanProps    & structural (type exports) \\
\texttt{FUNCTION}  & \ppTWOfunctionCoordMeanProps  & behavioral (input--output) \\
\texttt{INTERFACE} & \ppTWOinterfaceCoordMeanProps & structural (method signatures) \\
\midrule
\textbf{Overall avg.} & \textbf{\ppTWOoverallCoordMeanProps} & --- \\
\bottomrule
\end{tabular}
\end{table}

The uniform average of \ppTWOoverallCoordMeanProps{} propositions per coordinate
reflects the current site construction pipeline, which emits a fixed structural
and behavioral proposition pair for each coordinate.  As the proposition
engine matures, finer-grained extraction will differentiate proposition
counts across coordinate kinds; the macro infrastructure is in place
to track this progression automatically.
...
theoretical construct: all \compTrustTotal{} proof obligations are
solver-discharged across \compTotalPrograms{} programs (\compTrustSolverdischargedPct{}),
with individual judgment operations completing in \compMethodsMeanMs{}\,ms mean.
By retaining all eight components
