...
\newcommand{\coord}{\mathsf{c}}               % coordinate (already in common)
...
We integrate the framework into \JuGeo{}, implement the three exploration
strategies in \texttt{src/jugeo/generation/state\_space/}, connect them
to the local-construction subsystem in
\texttt{src/jugeo/generation/local\_construction/}, and formalise the
key correctness properties in \LEAN{}.  Experiments on a benchmark of
300 programs show that \BFS{} achieves full local coverage in
$1.8\times$ fewer rounds than \DFS{} on average, while \HEUR{} matches
\BFS{} coverage using only $43\,\%$ of the visited-state budget.
\end{abstract}
...
\BMC{} ($\depth=5$)& 148   & 4.7   & 0.91 & 11.2 & Bounded \\
\HEUR{}            & 207   & 6.9   & 1.00 & 17.8 & Full \\
\bottomrule
\end{tabular}
\end{table}

\BFS{} and \DFS{} achieve $\covfrac = 1.00$ on all programs (as
guaranteed by the theorem).  \HEUR{} matches full coverage while
exploring only $207/482 = 43\,\%$ of the states that \BFS{} requires.
\BMC{} at depth $5$ misses $9\,\%$ of patches on average; raising the
bound to $\depth = 10$ brings it to $\covfrac = 0.98$.

\subsection{Round efficiency}

\DFS{} requires $33.1$ orchestrator rounds on average vs.\ \BFS{}'s
$18.3$.  This is because \DFS{} visits deep states early, triggering
...
\texttt{COVERAGE\_FRAC} with $\theta = 0.90$.  On programs where the
last $10\,\%$ of patches require many high-depth states,
\texttt{COVERAGE\_FRAC} at $\theta = 0.90$ reduces the state budget by
$61\,\%$ with a coverage cost of only $0.10$.  For
