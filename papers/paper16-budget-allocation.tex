...
\begin{table}[h]
\centering
\caption{Strategy-level allocation outcomes across \numcases{} sessions.}
\label{tab:results}
\begin{tabular}{lccc}
\toprule
Strategy proxy & Verified & Mean time & Mean props \\
\midrule
\texttt{EAGER}      & \ppSixteenEagerVerified{}/\ppSixteenTotalPrograms{} & \ppSixteenEagerMeanTime{} & \ppSixteenEagerMeanProps{} \\
\texttt{EXHAUSTIVE} & \ppSixteenExhaustiveVerified{}/\ppSixteenTotalPrograms{} & \ppSixteenExhaustiveMeanTime{} & \ppSixteenExhaustiveMeanProps{} \\
\texttt{ITERATIVE}  & \ppSixteenIterativeVerified{}/\ppSixteenTotalPrograms{} & \ppSixteenIterativeMeanTime{} & \ppSixteenIterativeMeanProps{} \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:results} summarizes the three measured strategy proxies.
All three verify the full benchmark suite, with exhaustive search paying a
small time premium for broader search and iterative search serving as the
reference baseline for the workload ratios in Table~\ref{tab:breakdown}.
On the \expTotalPrograms-program benchmark, \expAccuracy{} of programs verify
successfully (mean time \expTimeMean).

\subsection{Breakdown by workload}

\begin{table}[h]
\centering
\caption{Per-workload time ratios relative to the iterative baseline.}
\label{tab:breakdown}
\begin{tabular}{lcc}
\toprule
Workload & Eager / Iterative & Exhaustive / Iterative \\
\midrule
Arithmetic  & \ppSixteenArithFixedRatio{} & \ppSixteenArithAdaptiveRatio{} \\
Structural  & \ppSixteenStructFixedRatio{} & \ppSixteenStructAdaptiveRatio{} \\
Mixed       & \ppSixteenMixedFixedRatio{} & \ppSixteenMixedAdaptiveRatio{} \\
\bottomrule
\end{tabular}
\end{table}

Arithmetic workloads stay close to the iterative baseline, while structural
workloads show the largest spread between eager and exhaustive search.  Mixed
workloads benefit most from the exhaustive proxy, reflecting the value of
rebalancing once arithmetic and structural obligations coexist in the same
session.
