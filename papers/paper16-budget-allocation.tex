...
\midrule
\texttt{FIXED}    & 61.2 & $30.0\,\mathrm{s}$ & 28.4 & 0 & 1.63 \\
\texttt{PRIORITY} & 74.5 & $30.0\,\mathrm{s}$ & 17.1 & 0 & 1.41 \\
\texttt{ADAPTIVE} & 88.7 & $29.4\,\mathrm{s}$ &  4.3 & 2.1 & 1.18 \\
\texttt{OFFLINE}  & 99.1 & $28.2\,\mathrm{s}$ &  0.9 & -- & 1.00 \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:results} shows that \AdaptAlloc{} achieves
$\ALG/\OPT = 1.18$ on average, well within the theoretical 2-competitive
bound.  The stall rate is reduced from 28.4\% (Fixed) to 4.3\%
(Adaptive), reflecting effective budget redistribution away from
exhausted channels.
...
\midrule
Arithmetic  & 1.02 & 1.03 & 1.01 & 1.00 \\
Structural  & 2.13 & 1.71 & 1.31 & 1.00 \\
Mixed       & 1.74 & 1.49 & 1.22 & 1.00 \\
\bottomrule
\end{tabular}
\end{table}
...
policy achieves $\ALG/\OPT = 2.13$ on structural workloads, nearly
saturating the 2-competitive upper bound, confirming our
lower bound of Corollary~\ref{cor:lower}.
...
$0.14\,\mathrm{ms}$ per round, negligible relative to solver
invocation times ($\medtime$).  Total orchestration overhead
