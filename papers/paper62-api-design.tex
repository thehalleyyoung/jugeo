...
% ─────────────────────────────────────────────────────────────────────
\begin{abstract}
We present a novel approach to evaluating and improving API surface design
using the topological structure of \JuGeo's semantic sites.  Traditional
API quality metrics focus on naming conventions, parameter counts, and
documentation coverage.  Our approach introduces \emph{cohomological API
metrics}: the \emph{cohesion index} $\CohIndex(\APISurface)$, measuring how
tightly the public interface's covering families bind its components; the
\emph{coupling index} $\CoupIndex(\APISurface)$, measuring cross-boundary
morphism density; and the \emph{exposure index} $\ExpIndex(\APISurface)$,
measuring the fraction of internal coordinates visible through the API
boundary $\APIBoundary$.  Using the \InterfRoute, \PubAlign, and \ProbAtlas\
API methods, we compute these metrics for \compTotalPrograms\ benchmark
programs spanning \compSiteTotalFunctions\ functions and
\compSiteTotalClasses\ classes.  We prove that high cohesion and low coupling
imply that the API boundary is a \emph{closed sub-site}, ensuring that
changes to internal coordinates do not leak through the public interface.
The Lean~4 formalization of this closure property is presented, and
experiments demonstrate that our metrics correlate with established
...
# Build the semantic site
site = SiteBuilder("mylib/__init__.py").build()

# Identify public coordinates
public_coords = [c for c in site.coordinates
                 if not c.name.startswith('_')]
print(f"Total coordinates: {len(site.coordinates)}")
print(f"Public coordinates: {len(public_coords)}")

# Classify morphisms
internal, boundary, external = [], [], []
for m in site.morphisms():
    src_pub = m.source in public_coords
    tgt_pub = m.target in public_coords
    if src_pub and tgt_pub:     external.append(m)
    elif src_pub or tgt_pub:    boundary.append(m)
    else:                       internal.append(m)

print(f"Internal: {len(internal)}, "
      f"Boundary: {len(boundary)}, "
      f"External: {len(external)}")
# Output: Total coordinates: 12
#         Public coordinates: 5
#         Internal: 8, Boundary: 3, External: 6
\end{lstlisting}

In this example, the 5~public coordinates out of 12~total give an exposure
index of $\ExpIndex = 5/12 \approx 0.42$.  The 6~external morphisms out
of $\binom{5}{2} = 10$ possible pairs give $\CohIndex = 0.60$.  The
3~boundary morphisms out of $8 + 3 + 6 = 17$ total yield
$\CoupIndex = 3/17 \approx 0.18$.
...
# Build site and configure engine
site = SiteBuilder("mylib/__init__.py").build()
...
public = {c for c in site.coordinates
          if not c.name.startswith('_')}
private = set(site.coordinates) - public

# Classify morphisms
ext = [m for m in site.morphisms()
       if m.source in public and m.target in public]
bdy = [m for m in site.morphisms()
       if (m.source in public) != (m.target in public)]
total_m = len(site.morphisms())

# Primary metrics
n = len(public)
coh = len(ext) / (n * (n - 1) / 2) if n > 1 else 1.0
coup = len(bdy) / total_m if total_m > 0 else 0.0
exp = n / len(site.coordinates)

# Derived: API complexity
api_comp = sum(1 for m in site.morphisms()
               if m.source in public
               and m.target not in public)

# Derived: Interface entropy
degrees = {}
for m in ext:
...
if D > 0:
    for p in public:
        d = degrees.get(p, 0)
        if d > 0:
            entropy -= (d / D) * math.log2(d / D)

print(f"Cohesion: {coh:.3f}, Coupling: {coup:.3f}")
print(f"Exposure: {exp:.3f}")
print(f"API Complexity: {api_comp}")
print(f"Interface Entropy: {entropy:.3f}")
\end{lstlisting}
