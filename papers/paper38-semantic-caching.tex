...
a complete, sorry-free proof.  Empirically, the current benchmark is
limited: across \ppXXXVIIITotalPrograms{} programs the observed hit rate
is \ppXXXVIIIHitRate{} and the overall speedup is
\ppXXXVIIISpeedup.  This audited version therefore keeps the
cache-correctness claim but not the stronger latency-win claim.
\end{abstract}
...
\begin{lstlisting}[language=Python, caption={Querying the semantic closure of a verification site with incremental caching.}]
from jugeo.geometry import SiteBuilder

code = open("web_server.py").read()
site = SiteBuilder(code).build()

print(f"Coordinates:       {site.coordinate_count()}")
print(f"Covering families: {len(site.covering_families())}")

# Compute the semantic closure (cache-aware transitive closure)
closure = site.semantic_closure()
print(f"Cache hits:        {closure['cache_hit_count']}")
print(f"Cache misses:      {closure['cache_miss_count']}")
hit_rate = (closure['cache_hit_count']
            / max(closure['cache_hit_count']
                  + closure['cache_miss_count'], 1))
print(f"Hit rate:          {hit_rate * 100:.1f}%")
print(f"Latency (cached):  {closure['latency_ms']:.2f} ms")
print(f"Speedup factor:    {closure['speedup_factor']:.2f}x")
\end{lstlisting}
...
The following snippet shows how to instantiate a cache policy and
insert or retrieve entries through the Python API:

\begin{lstlisting}[style=jugeo-python,
  caption={Configuring a semantic cache policy and querying cached
           verification results.},
  label={lst:cache-policy-api}]
from jugeo.cache import CachePolicy, SemanticCache

# Configure an LRU-based cache with capacity 10,000
policy = CachePolicy(
    strategy="LRU",
    max_entries=10_000,
    ttl_seconds=3600,
...
# Insert a verification result keyed by code + spec hashes
cache.put(code_hash="a3f1b8", spec_hash="b7d20e",
          result="Valid", proof_obligations=12)

entry = cache.get(code_hash="a3f1b8", spec_hash="b7d20e")
print(f"Result: {entry.result}")      # Valid
print(f"Fresh:  {entry.is_fresh()}")   # True
\end{lstlisting}
...
We evaluate the semantic cache on the 10-program benchmark reported in
\texttt{data-paper38.tex}.  The audited data records cache-warming key
counts and hit counts for three triggers (\texttt{FileEdit},
\texttt{ImportChange}, and \texttt{ScheduledSweep}) together with the
overall hit rate and speedup.
...
\midrule
\textbf{Overall} & \textbf{\ppXXXVIIITotalPrograms} & \multicolumn{3}{c}{\emph{Observed hit rate \ppXXXVIIIHitRate{}; theorem covers correctness, not speed}} \\
\bottomrule
\end{tabularx}
\end{table}

Table~\ref{tab:hit-rates} summarises the available cache behaviour
measurements.  Cache correctness remains a theorem-level property;
practically, the observed hit rate on the current benchmark is low.

\subsection{Latency reduction}

\begin{table}[t]
\centering
\caption{Observed latency summary for the audited cache benchmark.}
\label{tab:latency}
...
\midrule
\LRU{}  & --- & --- & \ppXXXVIIISpeedup \\
\LFU{}  & --- & --- & \ppXXXVIIISpeedup \\
\FIFO{} & --- & --- & \ppXXXVIIISpeedup \\
\TTL{}  & --- & --- & \ppXXXVIIISpeedup \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:latency} should be read conservatively: the observed
overall speedup is \ppXXXVIIISpeedup, so the current benchmark does not
show a meaningful latency gain from caching.
...
\midrule
FileEdit         & \ppXXXVIIIFileEditKeys & \ppXXXVIIIFileEditHits & \subSiteMeanTime \\
ImportChange     & \ppXXXVIIIImportChangeKeys & \ppXXXVIIIImportChangeHits & \subSiteMeanTime \\
ScheduledSweep   & \ppXXXVIIIScheduledSweepKeys & \ppXXXVIIIScheduledSweepHits & \subSiteMeanTime \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:warming} shows that \texttt{FileEdit} warms the most keys
and also produces the most extra hits, but the absolute hit counts are
still small.

\subsection{Checkpoint overhead}

The current benchmark does not provide a separate validated checkpoint
timing measurement, so we make no quantitative checkpoint-overhead claim
here.
...
The current benchmark does not support a performance-win claim over the
listed baselines; the observed overall speedup remains
\ppXXXVIIISpeedup.  The Cache Correctness Theorem is about soundness of
served entries, not about outperforming every baseline.

Listing~\ref{lst:cache-benchmark} shows the Python API used to run
the benchmark and collect the hit-rate and warming statistics reported
in Tables~\ref{tab:hit-rates}--\ref{tab:warming}.

\begin{lstlisting}[style=jugeo-python,
  caption={Measuring cache hit rates and warming effectiveness across
           the benchmark suite.},
  label={lst:cache-benchmark}]
from jugeo.cache import SemanticCache, CachePolicy
from jugeo.geometry import SiteBuilder

policy = CachePolicy(strategy="LRU", max_entries=10_000)
cache  = SemanticCache(policy=policy)

# Run verification across all benchmark programs
programs = load_benchmark("paper38_programs.json")
for prog in programs:
    site   = SiteBuilder(prog.source).build()
    result = site.verify(cache=cache)
...
# Inspect warming effectiveness by trigger kind
for trigger in ["FileEdit", "ImportChange", "ScheduledSweep"]:
    w = stats.warming[trigger]
    print(f"{trigger}: {w.keys_warmed} keys, "
          f"{w.extra_hits} extra hits")
\end{lstlisting}
...
\item The Cache Correctness Theorem
  ...
  formal \LEAN{} proof.
\item Empirical validation on \ppXXXVIIITotalPrograms{} benchmark
  programs showing a low observed hit rate (\ppXXXVIIIHitRate{}) and no
  meaningful speedup (\ppXXXVIIISpeedup), while remaining consistent
  with the theorem's soundness requirements.
\end{enumerate}
