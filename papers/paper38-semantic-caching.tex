...
average verification latency by $5.9{\times}$ on a suite of $300$
programs under a realistic edit-compile-verify loop, with $89\%$ of
...
\item \textbf{Cache Correctness Theorem}
  ...
  entries are sound.
\item \textbf{Experiments} (\S\ref{sec:experiments}): $5.9{\times}$
  median speedup, $89\%$ hit rate.
\end{enumerate}
...
\midrule
Sorting        & 30  & 93.2\%  & 88.4\%  & 71.2\% \\
Graph alg.     & 30  & 91.7\%  & 86.9\%  & 68.4\% \\
Numerical      & 30  & 94.1\%  & 90.2\%  & 74.8\% \\
Parsers        & 30  & 89.4\%  & 84.6\%  & 66.1\% \\
Web frameworks & 30  & 87.2\%  & 82.1\%  & 63.5\% \\
Compilers      & 30  & 91.3\%  & 87.8\%  & 70.9\% \\
Cryptography   & 30  & 95.6\%  & 91.0\%  & 77.3\% \\
Data structs   & 30  & 92.8\%  & 88.6\%  & 72.0\% \\
ML utilities   & 30  & 88.9\%  & 84.3\%  & 65.8\% \\
System utils   & 30  & 90.5\%  & 85.9\%  & 67.4\% \\
\midrule
\textbf{Overall} & \textbf{300} & \textbf{91.5\%} &
\textbf{86.9\%} & \textbf{69.7\%} \\
\bottomrule
\end{tabularx}
\end{table}

Table~\ref{tab:hit-rates} shows hit rates for the three cascade
strategies.  \texttt{Transitive} achieves an overall hit rate of
$86.9\%$, slightly below \texttt{DirectOnly} ($91.5\%$) but correctly
handling all indirect-dependency invalidations.
\texttt{Conservative} sacrifices hit rate ($69.7\%$) for maximum
safety.
...
\midrule
\LRU{}  & 38.4 & 6.3 & $6.1\times$ \\
\LFU{}  & 38.4 & 6.5 & $5.9\times$ \\
\FIFO{} & 38.4 & 6.8 & $5.6\times$ \\
\TTL{}  & 38.4 & 7.1 & $5.4\times$ \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:latency} shows median per-query latency.  \LRU{}
achieves the highest speedup ($6.1\times$) because it retains the
most recently accessed entries, which are the most likely to be
queried again.  The overall $5.9\times$ figure in the abstract uses
the \LFU{} strategy, which is slightly more robust to adversarial
access patterns.
...
\midrule
FileEdit         & 12.3 avg & 8.7 avg  & 47.2 \\
ImportChange     & 34.6 avg & 22.1 avg & 133.0 \\
ScheduledSweep   & 8.9 avg  & 6.2 avg  & 34.1 \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:warming} shows that the \texttt{FileEdit} trigger
provides the best cost-benefit ratio, warming $12.3$ keys on average
and generating $8.7$ additional cache hits at a cost of $47.2$\,ms of
idle-time computation.

\subsection{Checkpoint overhead}

Checkpoint creation via \texttt{CheckpointBuilder} adds a median
overhead of $2.1$\,ms per checkpoint (JSON format) or $0.4$\,ms
(MessagePack).  Restoration from a MessagePack checkpoint takes
$1.8$\,ms median for a $1{,}000$-entry cache.  These figures confirm
...
Our semantic cache outperforms all baselines: $5.9\times$ speedup vs.\
$1.0\times$ (no cache), $2.7\times$ (file-hash), and $4.1\times$
(AST-hash).
...
\item The Cache Correctness Theorem
  ...
  formal \LEAN{} proof.
\item Empirical validation on $300$ programs showing $5.9\times$
  median latency reduction and $86.9\%$ hit rate under the
