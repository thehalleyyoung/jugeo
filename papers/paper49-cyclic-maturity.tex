...
\newcommand{\Proto}{\mathsf{PROTO}}
\newcommand{\Oper}{\mathsf{OPER}}
\newcommand{\Fed}{\mathsf{FED}}
\newcommand{\SelfImp}{\mathsf{SELFIMP}}
\newcommand{\Matur}{\mathsf{MATURE}}
...
incidents by $73\%$ and increases verifier throughput by $2.4{\times}$ over
...
\begin{tabular}{cccc}
\toprule
\textbf{Cycle} & \textbf{Level} & \textbf{Score} & \textbf{Sig.\ cycles} \\
\midrule
0 (baseline) & \Proto{} (0) & 18.3 & 0 \\
1            & \Oper{} (1)  & 42.7 & 3 \\
2            & \Fed{} (2)   & 67.1 & 8 \\
3            & \SelfImp{} (3) & 81.9 & 14 \\
\bottomrule
...
Pass rate increased by $30.4$ percentage points from baseline to cycle 3.
Median latency decreased from $14.2\,\mathrm{ms}$ to $5.9\,\mathrm{ms}$
($2.4{\times}$ speedup), attributed primarily to the \textsc{Performance}
improvements added by \texttt{CapabilityExpander} in cycles 1 and 2.
Stale-doctrine incidents dropped by $73\%$ from 112 to 30, confirming that
\textsc{Coverage} expansions are effective.

\subsection{Federation Overhead}

We measured the overhead of the federation synchronisation for a three-node
cluster.  The majority-quorum consensus round adds on average $1.3\,\mathrm{ms}$
per cycle, and the \texttt{PeerSynchronizer} diff/patch step adds
$0.4\,\mathrm{ms}$ per peer.  Total federation overhead per cycle is
$1.3 + 2 \times 0.4 = 2.1\,\mathrm{ms}$, which is less than $0.15\%$ of the
total verification budget.
...
\begin{enumerate}
  \item \textbf{No capability expansion}: \texttt{CapabilityExpander.expand}
    always returns \texttt{False}.  Pass rate saturates at $64\%$ after cycle 1.
  \item \textbf{No federation}: single-node deployment with no consensus.
    ...
    in cycle 3 (pass rate $86.1\%$ vs.\ $91.7\%$).
