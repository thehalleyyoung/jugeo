...
Proposition accuracy   & $\expPropsOkSum{}/\expPropsSum{}$ \\
...
\midrule
Localised to coordinate & \checkmark & \ppTENfstarLocalised & partial & \ppTENleanLocalised \\
Counterexample witness  & \checkmark & \ppTENfstarCounterex & \checkmark & \ppTENleanCounterex \\
Repair suggestion       & \checkmark & \ppTENfstarRepair & \ppTENdafnyRepair & \ppTENleanRepair \\
Trust annotation        & \checkmark & \ppTENfstarTrustAnnot & \ppTENdafnyTrustAnnot & \ppTENleanTrustAnnot \\
Overlap context         & \checkmark & \ppTENfstarOverlap & \ppTENdafnyOverlap & \ppTENleanOverlap \\
\bottomrule
\end{tabular}
\end{table}
...
\Fstar{} & \ppTENfstarTrustLevels & \ppTENfstarAlgebra & \ppTENfstarProvenance \\
\Dafny{} & \ppTENdafnyTrustLevels & \ppTENdafnyAlgebra & \ppTENdafnyProvenance \\
\LEAN{}  & \ppTENleanTrustLevels & \ppTENleanAlgebra & \ppTENleanProvenance \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Proof burden comparison for merge sort verification.
  \textsuperscript{$\dagger$}Measurement pending; real tool comparison experiments in progress.}\label{tab:burden}
\begin{tabular}{@{}l r r r r@{}}
\toprule
...
\midrule
\JuGeo{} & 19 &  0 & 0  & 19 \\
\Dafny{} & ---\textsuperscript{$\dagger$} & ---\textsuperscript{$\dagger$} & ---\textsuperscript{$\dagger$} & ---\textsuperscript{$\dagger$} \\
\Fstar{} & ---\textsuperscript{$\dagger$} & ---\textsuperscript{$\dagger$} & ---\textsuperscript{$\dagger$} & ---\textsuperscript{$\dagger$} \\
\LEAN{}  & ---\textsuperscript{$\dagger$} & ---\textsuperscript{$\dagger$} & ---\textsuperscript{$\dagger$} & ---\textsuperscript{$\dagger$} \\
\bottomrule
\end{tabular}
\end{table}
...
Annotation-free         & \checkmark & \ppTENfstarAnnotFree & \ppTENdafnyAnnotFree & \ppTENleanAnnotFree \\
...
Certificate generation  & \checkmark & \ppTENfstarCertGen & \ppTENdafnyCertGen & \checkmark \\
...
builder = SiteBuilder("web_router")
c_mod = Coordinate(("web_router",), kind=CoordinateKind.MODULE)
c_router = Coordinate(("web_router", "Router"), kind=CoordinateKind.FUNCTION)
c_handle = Coordinate(
    ("web_router", "Router", "handle"),
    kind=CoordinateKind.FUNCTION,
)
c_limiter = Coordinate(
...
built_site = (
    ...
    .add_coordinate(c_router)
    .add_coordinate(c_handle)
    .add_coordinate(c_limiter)
    .add_coordinate(c_iface)
    .add_coordinate(c_try)
    .add_morphism(Morphism(c_router, c_mod, MorphismKind.RESTRICTION))
    .add_morphism(Morphism(c_handle, c_router, MorphismKind.RESTRICTION))
    .add_morphism(Morphism(c_try, c_handle, MorphismKind.RESTRICTION))
    .add_morphism(Morphism(c_limiter, c_mod, MorphismKind.RESTRICTION))
    .add_morphism(Morphism(c_handle, c_limiter, MorphismKind.TRANSPORT))
    .add_morphism(Morphism(c_iface, c_mod, MorphismKind.RESTRICTION))
    .build()
)
...
j1 = (
    ...
    .claiming(Proposition(
        ...
        formula="handle returns valid HTTP status",
    ))
    .of_type_named("HttpResponseContract")
...
\midrule
\textsc{Structural}  & \suppPropStructuralCount & \suppPropStructuralPct \\
\textsc{Behavioral}  & \suppPropBehavioralCount & \suppPropBehavioralPct \\
\textsc{Relational}  & \suppPropRelationalCount & \suppPropRelationalPct \\
\textsc{Resource}    & \suppPropResourceCount & \suppPropResourcePct \\
\textsc{Semantic}    & \suppPropSemanticCount & \suppPropSemanticPct \\
\midrule
\textbf{Total}       & \textbf{\suppPropTotal{}} & 100\% \\
\bottomrule
\end{tabular}
\end{table}
...
together accounting for 86.0\% of all \suppPropTotal{} propositions.  Relational
propositions (\suppPropRelationalPct{}) appear primarily in the web domain (cross-module
interface checks), while resource (\suppPropResourcePct{}) and semantic
(\suppPropSemanticPct{}) propositions are rare in this benchmark.
...
\midrule
\textsc{Module}    & \suppCoordModuleCount & \suppCoordModulePct \\
\textsc{Function}  & \suppCoordFunctionCount & \suppCoordFunctionPct \\
\textsc{Interface} & \suppCoordInterfaceCount & \suppCoordInterfacePct \\
\midrule
\textbf{Total}     & \textbf{\suppCoordTotal{}} & 100\% \\
\bottomrule
\end{tabular}
\end{table}

Function coordinates dominate at \suppCoordFunctionPct{} (\suppCoordFunctionCount{}),
followed by module coordinates at \suppCoordModulePct{} (\suppCoordModuleCount{})
and interface coordinates at \suppCoordInterfacePct{} (\suppCoordInterfaceCount{}).
The total coordinate count of \suppCoordTotal{}
across \compTotalPrograms{} programs gives a mean of \compCoordsMean{}
per program.
...
\midrule
\textsc{Restriction} & \compMorphRestriction & \compMorphRestrictionPct \\
\textsc{Inclusion}   & \compMorphInclusion & \compMorphInclusionPct \\
\textsc{Transport}   & \compMorphTransport & \compMorphTransportPct \\
\midrule
\textbf{Total}       & \textbf{\compMorphTotal} & \textbf{100\%} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{tabular}{@{}l r r r@{}}
\toprule
\textbf{Strategy} & \textbf{Mean (ms)} & \textbf{Runs}
  & \textbf{Success Rate} \\
\midrule
\texttt{EAGER}      & \compDescentEagerMeanMs & \compDescentEagerRuns & \compDescentEagerRate \\
\texttt{EXHAUSTIVE} & \compDescentExhaustiveMeanMs & \compDescentExhaustiveRuns & \compDescentExhaustiveRate \\
\texttt{ITERATIVE}  & \compDescentIterativeMeanMs & \compDescentIterativeRuns & \compDescentIterativeRate \\
\texttt{OPTIMISTIC} & \compDescentOptimisticMeanMs & \compDescentOptimisticRuns & \compDescentOptimisticRate \\
\bottomrule
\end{tabular}
\end{table}

All four descent strategies achieve \compDescentEagerRate{} success
on our benchmark, with sub-millisecond descent times.
The overall CLI time (\compTimeMean) is dominated by the SMT solver,
not the descent engine.  Strategy choice therefore has negligible
impact on end-to-end latency for these program sizes.
